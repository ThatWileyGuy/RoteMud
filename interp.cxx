...
void interpret(CHAR_DATA* ch, char* argument)
{
	...
	if (!IS_NPC(ch) && fLogPC)
	{
		...
		if (ch->desc && ch->desc->original)
			sprintf_s(log_buf, "Log %s (%s): %s", ch->name,
	...
	*/
	...
	if ((!IS_NPC(ch) && IS_SET(ch->act, PLR_SLOG)))
	{
		/* Added by Narn to show who is switched into a mob that executes
		   a logged command.  Check for descriptor in case force is used. */
		if (ch->desc && ch->desc->original)
			sprintf_s(log_buf, "SecretLog %s (%s): %s", ch->name,
				ch->desc->original->name, logline);
		else
	...
	if (time_used > std::chrono::seconds(1))
	{
		...
		sprintf_s(log_buf, "[*****] LAG: %s: %s %s (R:%d S:%d.%06d)", ch->name,
...
void send_timer(struct timerset* vtime, CHAR_DATA* ch)
{
	...
	ch_printf(ch, "Time (in secs): min %d.%0.6d; avg: %d.%0.6d; max %d.%0.6d"
		"\n\r", duration_cast<seconds>(vtime->min_time), duration_cast<microseconds>(vtime->min_time),
...
{
	userec->num_uses++;
	if (userec->num_uses == 1
		|| time_used < userec->min_time)
	{
		userec->min_time = time_used;
	}
	if (userec->num_uses == 1
		|| time_used > userec->max_time)
	{
