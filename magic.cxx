...
ch_ret spell_locate_object(int sn, int level, CHAR_DATA* ch, void* vo)
{
	...
	for (obj = first_object; obj; obj = obj->next)
	{
		...
		if (cnt >= MAX_NEST)
		{
			sprintf_s(buf, "spell_locate_obj: object [%d] %s is nested more than %d times!",
...
ch_ret spell_forcepush(int sn, int level, CHAR_DATA* ch, void* vo)
{
	...
	act(AT_BLUE, "You extend your arm in $N's direction and send $M flying into a near by wall with a push of the force.", ch, NULL, victim, TO_CHAR);
	act(AT_BLUE, "$n extends $s hand towards you and you feel yourself sent tumbling into a nearby wall.", ch, NULL, victim, TO_VICT);
	act(AT_BLUE, "$n extends $s hand towards $N and sends $N tumbling into a near by wall with a push of the force.", ch, NULL, victim, TO_NOTVICT);
