...
char* format_obj_to_char(OBJ_DATA* obj, CHAR_DATA* ch, bool fShort)
{
	...
	buf[0] = '\0';
	if (IS_OBJ_STAT(obj, ITEM_INVIS))   strcat_s(buf, "(Invis) ");
	if ((IS_AFFECTED(ch, AFF_DETECT_MAGIC) || IS_IMMORTAL(ch))
		&& IS_OBJ_STAT(obj, ITEM_MAGIC))   strcat_s(buf, "&B(Blue Aura)&w ");
	if (IS_OBJ_STAT(obj, ITEM_GLOW))   strcat_s(buf, "&G&W(&OG&Yl&wo&Ww&wi&Yn&Og&G&W) ");
	if (IS_OBJ_STAT(obj, ITEM_HUM))   strcat_s(buf, "&G&W(&gH&Gu&wm&Wm&wi&Gn&gg&G&W) ");
	if (IS_OBJ_STAT(obj, ITEM_HIDDEN))   strcat_s(buf, "(Hidden) ");
	if (IS_OBJ_STAT(obj, ITEM_BURRIED))   strcat_s(buf, "(Burried) ");
	if (IS_IMMORTAL(ch)
		&& IS_OBJ_STAT(obj, ITEM_PROTOTYPE)) strcat_s(buf, "(PROTO) ");
	if (IS_AFFECTED(ch, AFF_DETECTTRAPS)
		&& is_trapped(obj))   strcat_s(buf, "(Trap) ");
...
void show_char_to_char_0(CHAR_DATA* victim, CHAR_DATA* ch)
{
	...
	if (!IS_NPC(victim) && !victim->desc)
	{
		if (!victim->switched)		strcat_s(buf, "(Link Dead) ");
		else
			if (!IS_AFFECTED(victim->switched, AFF_POSSESS))
				strcat_s(buf, "(Switched) ");
	}
	if (!IS_NPC(victim)
		&& IS_SET(victim->act, PLR_AFK))		strcat_s(buf, "[AFK] ");
	if ((!IS_NPC(victim) && IS_SET(victim->act, PLR_WIZINVIS))
		|| (IS_NPC(victim) && IS_SET(victim->act, ACT_MOBINVIS)))
	{
		if (!IS_NPC(victim))
			sprintf_s(buf1, "(Invis %d) ", victim->pcdata->wizinvis);
		else sprintf_s(buf1, "(Mobinvis %d) ", victim->mobinvis);
		strcat_s(buf, buf1);
	}
	if (IS_AFFECTED(victim, AFF_INVISIBLE)) strcat_s(buf, "(Invis) ");
	if (IS_AFFECTED(victim, AFF_HIDE)) strcat_s(buf, "(Hide) ");
	if (IS_AFFECTED(victim, AFF_PASS_DOOR)) strcat_s(buf, "(Translucent) ");
	if (IS_AFFECTED(victim, AFF_FAERIE_FIRE)) strcat_s(buf, "&P(Pink Aura)&w ");
	if (!IS_NPC(victim) && IS_SET(victim->pcdata->act2, ACT_GAGGED)) strcat_s(buf, "&w&W(&rG&Ra&rg&Rg&re&Rd&w&W)&w ");
	if (!IS_NPC(victim) && IS_SET(victim->pcdata->act2, ACT_BOUND)) strcat_s(buf, "&G&W(&gB&Go&wu&Gn&gd&G&W)&w ");
	if (IS_EVIL(victim)
		&& IS_AFFECTED(ch, AFF_DETECT_EVIL)) strcat_s(buf, "&R(Red Aura)&w ");
	if ((victim->mana > 10)
	...
	if (IS_NPC(victim) && IS_IMMORTAL(ch)
		&& IS_SET(victim->act, ACT_PROTOTYPE)) strcat_s(buf, "(PROTO) ");
	...
	{
	case POS_DEAD:     strcat_s(buf, " is DEAD!!");			break;
	case POS_MORTAL:   strcat_s(buf, " is mortally wounded.");		break;
	case POS_INCAP:    strcat_s(buf, " is incapacitated.");		break;
	case POS_STUNNED:  strcat_s(buf, " is lying here stunned.");	break;
	...
	case POS_SLEEPING:
		if (victim->on != NULL)
		{
			...
			{
				sprintf_s(message, " is sleeping at %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
			else if (IS_SET(victim->on->value[2], SLEEP_ON))
			{
				sprintf_s(message, " is sleeping on %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
			else
			{
				sprintf_s(message, " is sleeping in %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
		}
	...
	case POS_RESTING:
		if (victim->on != NULL)
		{
			...
			{
				sprintf_s(message, " is resting at %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
			else if (IS_SET(victim->on->value[2], REST_ON))
			{
				sprintf_s(message, " is resting on %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
			else
			{
				sprintf_s(message, " is resting in %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
		}
	...
	case POS_SITTING:
		if (victim->on != NULL)
		{
			...
			{
				sprintf_s(message, " is sitting at %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
			else if (IS_SET(victim->on->value[2], SIT_ON))
			{
				sprintf_s(message, " is sitting on %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
			else
			{
				sprintf_s(message, " is sitting in %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
		}
	...
	case POS_STANDING:
		if (victim->on != NULL)
		{
			...
			{
				sprintf_s(message, " is standing at %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
			else if (IS_SET(victim->on->value[2], STAND_ON))
			{
				sprintf_s(message, " is standing on %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
			else
			{
				sprintf_s(message, " is standing in %s.",
					victim->on->short_descr);
				strcat_s(buf, message);
			}
		}
	...
	case POS_SHOVE:    strcat_s(buf, " is being shoved around.");	break;
	case POS_DRAG:     strcat_s(buf, " is being dragged around.");	break;
	case POS_MOUNTED:
...
void show_condition(CHAR_DATA* ch, CHAR_DATA* victim)
{
	...
	if ((IS_NPC(victim) && IS_SET(victim->act, ACT_DROID)) || IS_DROID(victim))
	{
		...
		if (percent >= 100) strcat_s(buf, " is in perfect condition.\n\r");
		else if (percent >= 90) strcat_s(buf, " is slightly scratched.\n\r");
		else if (percent >= 80) strcat_s(buf, " has a few scrapes.\n\r");
		else if (percent >= 70) strcat_s(buf, " has some dents.\n\r");
		else if (percent >= 60) strcat_s(buf, " has a couple holes in its plating.\n\r");
		else if (percent >= 50) strcat_s(buf, " has a many broken pieces.\n\r");
		else if (percent >= 40) strcat_s(buf, " has many exposed circuits.\n\r");
		else if (percent >= 30) strcat_s(buf, " is leaking oil.\n\r");
		else if (percent >= 20) strcat_s(buf, " has smoke coming out of it.\n\r");
		else if (percent >= 10) strcat_s(buf, " is almost completely broken.\n\r");
		else                       strcat_s(buf, " is about to EXPLODE.\n\r");

	}
	else
	{

		if (percent >= 100) strcat_s(buf, " is in perfect health.\n\r");
		else if (percent >= 90) strcat_s(buf, " is slightly scratched.\n\r");
		else if (percent >= 80) strcat_s(buf, " has a few bruises.\n\r");
		else if (percent >= 70) strcat_s(buf, " has some cuts.\n\r");
		else if (percent >= 60) strcat_s(buf, " has several wounds.\n\r");
		else if (percent >= 50) strcat_s(buf, " has many nasty wounds.\n\r");
		else if (percent >= 40) strcat_s(buf, " is bleeding freely.\n\r");
		else if (percent >= 30) strcat_s(buf, " is covered in blood.\n\r");
		else if (percent >= 20) strcat_s(buf, " is leaking guts.\n\r");
		else if (percent >= 10) strcat_s(buf, " is almost dead.\n\r");
		else                       strcat_s(buf, " is DYING.\n\r");
...
void do_examine(CHAR_DATA* ch, char* argument)
{
	...
	if ((obj = get_obj_here(ch, arg)) != NULL)
	{
		...
		case ITEM_ARMOR:
			...
			if (dam >= 10) strcat_s(buf, "in superb condition.");
			else if (dam == 9) strcat_s(buf, "in very good condition.");
			else if (dam == 8) strcat_s(buf, "in good shape.");
			else if (dam == 7) strcat_s(buf, "showing a bit of wear.");
			else if (dam == 6) strcat_s(buf, "a little run down.");
			else if (dam == 5) strcat_s(buf, "in need of repair.");
			else if (dam == 4) strcat_s(buf, "in great need of repair.");
			else if (dam == 3) strcat_s(buf, "in dire need of repair.");
			else if (dam == 2) strcat_s(buf, "very badly worn.");
			else if (dam == 1) strcat_s(buf, "practically worthless.");
			else if (dam <= 0) strcat_s(buf, "broken.");
			strcat_s(buf, "\n\r");
			send_to_char(buf, ch);
			break;
		...
		case ITEM_WEAPON:
			...
			strcpy_s(buf, "As you look more closely, you notice that it is ");
			if (dam == 0) strcat_s(buf, "in superb condition.");
			else if (dam == 1) strcat_s(buf, "in excellent condition.");
			else if (dam == 2) strcat_s(buf, "in very good condition.");
			else if (dam == 3) strcat_s(buf, "in good shape.");
			else if (dam == 4) strcat_s(buf, "showing a bit of wear.");
			else if (dam == 5) strcat_s(buf, "a little run down.");
			else if (dam == 6) strcat_s(buf, "in need of repair.");
			else if (dam == 7) strcat_s(buf, "in great need of repair.");
			else if (dam == 8) strcat_s(buf, "in dire need of repair.");
			else if (dam == 9) strcat_s(buf, "very badly worn.");
			else if (dam == 10) strcat_s(buf, "practically worthless.");
			else if (dam == 11) strcat_s(buf, "almost broken.");
			else if (dam == 12) strcat_s(buf, "broken.");
			strcat_s(buf, "\n\r");
			send_to_char(buf, ch);
		...
		case ITEM_FOOD:
			...
			if (dam >= 10) strcat_s(buf, "is fresh.");
			else if (dam == 9) strcat_s(buf, "is nearly fresh.");
			else if (dam == 8) strcat_s(buf, "is perfectly fine.");
			else if (dam == 7) strcat_s(buf, "looks good.");
			else if (dam == 6) strcat_s(buf, "looks ok.");
			else if (dam == 5) strcat_s(buf, "is a little stale.");
			else if (dam == 4) strcat_s(buf, "is a bit stale.");
			else if (dam == 3) strcat_s(buf, "smells slightly off.");
			else if (dam == 2) strcat_s(buf, "smells quite rank.");
			else if (dam == 1) strcat_s(buf, "smells revolting.");
			else if (dam <= 0) strcat_s(buf, "is crawling with maggots.");
			strcat_s(buf, "\n\r");
			send_to_char(buf, ch);
			break;
...
void do_exits(CHAR_DATA* ch, char* argument)
{
	...
	for (pexit = ch->in_room->first_exit; pexit; pexit = pexit->next)
	{
			...
			&& !IS_SET(pexit->exit_info, EX_HIDDEN))
		{
			...
			if (!fAuto)
			{
				if (IS_SET(pexit->exit_info, EX_CLOSED))
				{
					sprintf_s(insert_at, remaining, "%s%-5s - (closed)&w&W\n\r",
						color_str(AT_EXITS, ch), capitalize(dir_name[pexit->vdir]));
				}
				...
				else if (IS_SET(pexit->exit_info, EX_xAUTO))
				{
					...
					sprintf_s(insert_at, remaining, "%s%-5s - %s%s&w&W\n\r",
						color_str(AT_EXITS, ch), capitalize(dir_name[pexit->vdir]), color_str(AT_RMNAME, ch),
						room_is_dark(pexit->to_room)
						? "Too dark to tell"
						: pexit->to_room->name);
			}
			...
			{
				sprintf_s(insert_at, remaining, " %s&w&W",
...
{
	static char const* const sky_look[4] =
	{
	"cloudless",
	"cloudy",
	"rainy",
...
void do_who(CHAR_DATA* ch, char* argument)
{
	...
	*/
	...
	for (d = last_descriptor; d; d = d->prev)
	{
		...
		*/
		...
		if (!wch->rank || (wch->rank[0] == '\0')) 	//If the imm has set thier rank show it instead --Odis ->KeB
		{
			...
			case MAX_LEVEL - 1: race = "&G&z(&G&wHead Imm     &G&z)&G&w";	break;
			case MAX_LEVEL - 2: race = "&G&z(&G&wHigh Builder &G&z)&G&w";	break;
			case MAX_LEVEL - 3: race = "&G&z(&G&wBuilder      &G&z)&G&w";	break;
			case MAX_LEVEL - 4: race = "&G&z(&G&wApprentice   &G&z)&G&w";	break;
			case MAX_LEVEL - 5: race = "&G&z(&G&wRetired      &G&z)&G&w";	break;
			}
		}
	...
	if (first_newbie)
	{
		if (!ch)
		{
			sprintf_s(mudnamebuffer, "&G===&B[ &P%s &B]&G===&B[ &WMortals &B]&G=====================================================&B[]&W\n\n", sysdata.mud_acronym);
			fprintf(whoout, "%s", htmlcolor(mudnamebuffer));
		}

		else
		{
			sprintf_s(mudnamebuffer, "\n\r&G===&B[ &P%s &B]&G===&B[ &WMortals &B]&G=====================================================&B[]&W\n\r\n\r", sysdata.mud_acronym);
	...
	if (first_mortal)
	{
		if (!ch)
		{
			sprintf_s(mudnamebuffer, "&z(  &rPlayers&z  )&R-_-&r^^-_-^^-&R_-&r^^-_-^^-_-^^-&R_-^^-&r_-^^-_-&R^^&z(=====+ &r%s &z+====&z=)&w\n\n", sysdata.mudname);
			fprintf(whoout, "%s", htmlcolor(mudnamebuffer));
		}

		else
		{
			sprintf_s(mudnamebuffer, "\n\r&z(  &rPlayers&z  )&R-_-&r^^-_-^^-&R_-&r^^-_-^^-_-^^-&R_-^^-&r_-^^-_-&R^^&z(=====+ &r%s &z+====&z=)&w\n\r\n\r", sysdata.mudname);
			send_to_pager(mudnamebuffer, ch);
		}
	}
	...
	if (first_imm)
	{
		if (!ch)
		{
			sprintf_s(mudnamebuffer, "&z( &rImmortals&z )&R-_&r-^^-_-&R^&r-_-^^-_-^^&R-_-^^&r-_-^^-_-^^-&R_-^^&z(=====+ &r%s &z+====&z=)&w\n\n", sysdata.mudname);
			fprintf(whoout, "%s", htmlcolor(mudnamebuffer));
		}

		else
		{
			sprintf_s(mudnamebuffer, "\n\r&z( &rImmortals&z )&R-_&r-^^-_-&R^^&r-_-^^-_-^^&R-_-^^&r-_-^^-_-^^-&R_-^^&z(=====+ &r%s &z+====&z=)&w\n\r\n\r", sysdata.mudname);
			send_to_pager(mudnamebuffer, ch);
		}
	}
...
void do_setrank(CHAR_DATA* ch, char* argument)
{
	...
	{
		if (vict->rank) DISPOSE(vict->rank);
		vict->rank = str_dup("                  ");
...
void do_consider(CHAR_DATA* ch, char* argument)
{
	...
	*/
	...
	act(AT_CONSIDER, "You examine $N closely looking for any weaknesses.", ch, NULL, victim, TO_CHAR);
...
void do_practice(CHAR_DATA* ch, char* argument)
{
	...
	else
	{
		...
		if (sn == -1)
		{
			act(AT_TELL, "&R&C$n says:&W I've never heard of that one.",
				mob, NULL, ch, TO_VICT);
			return;
		}
		...
		if (skill_table[sn]->guild < 0 || skill_table[sn]->guild >= MAX_ABILITY)
		{
			act(AT_TELL, "&R&C$n says:&W I can't teach you that.",
				mob, NULL, ch, TO_VICT);
			return;
		}
		...
		{
			act(AT_TELL, "&R&C$n says:&W I don't think you're ready to learn that yet.",
				mob, NULL, ch, TO_VICT);
			return;
		}
		...
		{
			act(AT_TELL, "&R&C$n says:&W I don't know how to do that.",
				mob, NULL, ch, TO_VICT);
			return;
		}
		...
		{
			sprintf_s(buf, "$n tells you, 'I've taught you everything I can about %s.'",
				skill_table[sn]->name);
...
void do_config(CHAR_DATA* ch, char* argument)
{
	...
	if (arg[0] == '\0')
	{
		...
		send_to_char(IS_SET(ch->pcdata->flags, PCFLAG_MAP)
			? "[+COMPASS] You see the compass in your exits.\n\r"
		...
		if (nifty_is_name(argument, fellow->knownas))
		{
			for (vfellow = victim->first_fellow; vfellow; vfellow = vfellow->next)
			{
				if (!str_cmp(fellow->victim, vfellow->victim))
				{
					...
					ch_printf(victim, "%s describes %s to you. They sound quite similar to %s.n\r", PERS(ch, victim), fellow->knownas, vfellow->knownas);
	...
	for (pexit = ch->in_room->first_exit; pexit; pexit = pexit->next)
	{
			...
			&& !IS_SET(pexit->exit_info, EX_HIDDEN))
		{
			...
			if (!fAuto)
			{
				if (IS_SET(pexit->exit_info, EX_CLOSED))
				{
					sprintf_s(buf, "&G&W%-9s &R- (closed)",
						capitalize(dir_name[pexit->vdir]));
				}
				else if (IS_SET(pexit->exit_info, EX_WINDOW))
				{
					sprintf_s(buf, "&G&W%-9s &R- (window)",
						capitalize(dir_name[pexit->vdir]));
				}
				else if (IS_SET(pexit->exit_info, EX_xAUTO))
				{
					sprintf_s(buf, "&G&W%-9s &R- &G&W%s%s",
			...
			if (count == 1) strcpy_s(line0, buf);
			if (count == 2) strcpy_s(line1, buf);
			if (count == 3) strcpy_s(line2, buf);
			if (count == 4) strcpy_s(line3, buf);
			if (count == 5) strcpy_s(line4, buf);
			if (count == 6) strcpy_s(line5, buf);
			if (count == 7) strcpy_s(line6, buf);
			if (count == 8) strcpy_s(line7, buf);
			if (count == 9) strcpy_s(line8, buf);
			if (count == 10) strcpy_s(line9, buf);
		}
	}//end for
	if (!found)
