...
void do_bet(CHAR_DATA *ch, char *argument)
 {
   ...
   else if(ppl_in_arena)
   {
   ...
   else
      {
         ...
         send_to_char(buf, ch);
         sprintf_s(buf,"%s has placed %d credits on %s.", ch->name,
         newbet, ch->betted_on->name);
         sportschan(buf);
  }
}
 ...
 {   
    ...
    else
    {
      if(time_to_start >1)
      {
         ...
         sprintf_s(buf, "With just %d hours to go till the start of the arena %d people have accepted the challenge!\n\r", time_to_start, num_in_arena());
         sportschan (buf);
         sportschan (buf1);
      }
      else
      {
         ...
         sprintf_s(buf1, "%s\r\nType &Rarena &Wto enter.\r\n", buf1);
         sprintf_s(buf, "With just 1 hour to go till the start of the arena %d people have accepted the challenge!\n\r",  num_in_arena());
         sportschan (buf);
         sportschan (buf1);
      }
  ...
  if (!(ppl_in_arena))
  {
    ...
    else 
    {
      if(time_to_start >1)
      {
        sprintf_s(buf1, "The duel will start in %d hours.",
                        time_to_start);
      }
      else
      {
...
void do_game()
{
  ...
  {
     sprintf_s(buf, "With %d hours left in the game there are %d players left.",
             time_left_in_game, num_in_arena());
     sportschan(buf);
  }
  else if(time_left_in_game == 1)
  {
    sprintf_s(buf, "With 1 hour left in the game there are %d players left.",
                  num_in_arena());
    sportschan(buf);
  }
  ...
  for (d = first_descriptor; d; d = d->next)
  {
     if( d->connected == CON_PLAYING )
	  ...
	  phealth = (100 * tch->hit) / tch->max_hit;
          if (IS_SET(tch->in_room->room_flags2, ROOM_ARENA)
