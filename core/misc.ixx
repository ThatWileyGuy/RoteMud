...
void do_clone(CHAR_DATA* ch, char* argument)
{
    ...
    {
        strcpy_s(clanname, ch->pcdata->clan_name);
        STRFREE(ch->pcdata->clan_name);
        ch->pcdata->clan_name = STRALLOC("");
        if (ch->pcdata->bestowments)
        {
            strcpy_s(bestowments, ch->pcdata->bestowments);
...
void do_ammo(CHAR_DATA* ch, char* argument)
{
    ...
    if (wield->value[3] == WEAPON_BLASTER)
    {
        ...
        ch_printf(ch,
                  "You replace your ammunition cartridge.\n\rYour blaster is charged with %d shots at high power to %d "
                  "shots on low.\n\r",
                  charge / 5, charge);
...
void do_takedrug(CHAR_DATA* ch, char* argument)
{
    ...
    if (ch->fighting && number_percent() > (get_curr_dex(ch) * 2 + 48))
    {
        ...
        act(AT_MAGIC, "Oops... $p gets knocked from your hands rendering it completely useless!", ch, obj, nullptr,
            TO_CHAR);
    }
    else
    {
        ...
        case SPICE_RYLL:
            ...
            af.modifier = 1;
            af.duration =
                URANGE(1, 2 * (ch->pcdata->drug_level[drug] - ch->pcdata->addiction[drug]), 2 * obj->value[1]);
        ...
        case SPICE_ANDRIS:
            ...
            af.type = -1; // TODO this was an uninitialized use of sn - what should it actually be?
...
export void jedi_bonus(CHAR_DATA* ch)
{
...
export void sith_penalty(CHAR_DATA* ch)
{
    if (number_range(1, 100) == 1)
    {
        ch->max_mana++;
...
void pullorpush(CHAR_DATA* ch, OBJ_DATA* obj, bool pull)
{
    ...
    {
    default:
        sprintf_s(buf, "You can't %s that!\n\r", pull ? "pull" : "push");
        send_to_char(buf, ch);
        return;
        break;
    case ITEM_SWITCH:
    ...
    if (!oprog_use_trigger(ch, obj, nullptr, nullptr, nullptr))
    {
        sprintf_s(buf, "$n %s $p.", pull ? "pulls" : "pushes");
        act(AT_ACTION, buf, ch, obj, nullptr, TO_ROOM);
        sprintf_s(buf, "You %s $p.", pull ? "pull" : "push");
...
void do_smoke(CHAR_DATA* ch, char* argument)
{
    ...
    if (pipe->item_type != ITEM_PIPE)
    {
        ...
        act(AT_ACTION, "$n tries to smoke $p... (I wonder what $e's been putting his $s pipe?)", ch, pipe, nullptr,
            TO_ROOM);
        return;
    }
...
export void actiondesc(CHAR_DATA* ch, OBJ_DATA* obj, void* vo)
{
    ...
    *roomptr = '\0';
    ...
    /*
    sprintf_s( buf, "Charbuf: %s", charbuf );
    log_string_plus( buf, LOG_HIGH, LEVEL_LESSER );
    sprintf_s( buf, "Roombuf: %s", roombuf );
    log_string_plus( buf, LOG_HIGH, LEVEL_LESSER );
    */
...
void do_hail(CHAR_DATA* ch, char* argument)
{
    ...
    act(AT_ACTION, "$n $T", ch, nullptr, "arrives on a speederbike, gets off and pays the driver before it leaves.",
        TO_ROOM);
...
void do_train(CHAR_DATA* ch, char* argument)
{
    ...
    if (str_cmp(arg, "str") && str_cmp(arg, "strength") && str_cmp(arg, "dex") && str_cmp(arg, "dexterity") &&
    ...
    if (!str_cmp(arg, "str") || !str_cmp(arg, "strength"))
    {
        ...
        {
            act(AT_TELL, "$n tells you 'I cannot help you... you are already stronger than I.'", mob, nullptr, ch,
                TO_VICT);
            return;
        }
    ...
    if (!str_cmp(arg, "dex") || !str_cmp(arg, "dexterity"))
    {
        ...
        {
            act(AT_TELL, "$n tells you 'I cannot help you... you are already more dextrous than I.'", mob, nullptr, ch,
                TO_VICT);
            return;
        }
    ...
    if (!str_cmp(arg, "int") || !str_cmp(arg, "intelligence"))
    {
        ...
        {
            act(AT_TELL, "$n tells you 'I cannot help you... you are already more educated than I.'", mob, nullptr, ch,
                TO_VICT);
            return;
        }
    ...
    if (!str_cmp(arg, "wis") || !str_cmp(arg, "wisdom"))
    {
        ...
        {
            act(AT_TELL, "$n tells you 'I cannot help you... you are already far wiser than I.'", mob, nullptr, ch,
                TO_VICT);
            return;
        }
    ...
    if (!str_cmp(arg, "con") || !str_cmp(arg, "constitution"))
    {
        ...
        {
            act(AT_TELL, "$n tells you 'I cannot help you... you are already healthier than I.'", mob, nullptr, ch,
                TO_VICT);
            return;
        }
    ...
    if (!str_cmp(arg, "cha") || !str_cmp(arg, "charisma"))
    {
        ...
        {
            act(AT_TELL, "$n tells you 'I cannot help you... you already are more charming than I.'", mob, nullptr, ch,
                TO_VICT);
            return;
        }
    ...
    */
    ...
    if (!str_cmp(arg, "cha") || !str_cmp(arg, "charisma"))
    {
        ...
        send_to_char("&GYou spend some time focusing on how to improve your personality and feel \n\rmuch better about "
                     "yourself and the ways others see you.\n\r",
                     ch);
        ch->perm_cha++;
        return;
    }
...
void do_suicide(CHAR_DATA* ch, char* argument)
{
    ...
    if (strcmp(smaug_crypt(argument), ch->pcdata->pwd))
    {
        ...
        sprintf_s(logbuf, "%s attempting to commit suicide... WRONG PASSWORD!", ch->name);
        log_string(logbuf);
        return;
    }
    ...
    if (!wield)
    {
        act(AT_BLOOD, "You ram your thumbs into your eyesockets, blood spurts onto the floor.\n\r", ch, nullptr, nullptr,
            TO_CHAR);
        act(AT_BLOOD, "Blood spurts onto your shoes as $n rams his thumbs into $s eyesockets.", ch, nullptr, nullptr,
            TO_ROOM);
    }
    else
    {
        ...
        case WEAPON_BLASTER:
            act(AT_BLOOD,
                "You calmly place the barrel of your blaster into your mouth, and a spurt of brains falls to the "
                "ground behind you.",
                ch, nullptr, nullptr, TO_CHAR);
            act(AT_BLOOD, "A spurt of brains hits the ground as $n blows $s head off.", ch, nullptr, nullptr, TO_ROOM);
            break;

        case WEAPON_VIBRO_BLADE:
            act(AT_BLOOD, "With a sad determination and trembling hands you slit your own throat!", ch, nullptr, nullptr,
                TO_CHAR);
            act(AT_BLOOD, "Cold shivers run down your spine as you watch $n slit $s own throat!", ch, nullptr, nullptr,
                TO_ROOM);
            break;

        case WEAPON_FORCE_PIKE:
            act(AT_BLOOD, "You wedge your force pike into the ground and slam your face into the blade.", ch, nullptr,
                nullptr, TO_CHAR);
            act(AT_BLOOD,
                "$n slams $s face into $s force pike, pieces of face fall to the ground as the pike slices through it "
                "like butter.",
        ...
        case WEAPON_DUAL_LIGHTSABER:
            act(AT_BLOOD,
                "You calmly kneel, and hold your saber hilt to your sternum, and without a moments hesitation, turn it "
                "on.",
                ch, nullptr, nullptr, TO_CHAR);
            act(AT_BLOOD,
                "$n calmly kneels and holds $s saber hilt to $s chest, and turns it on. $e falls over in agony.", ch,
                nullptr, nullptr, TO_ROOM);
            break;

        case WEAPON_BOWCASTER:
            act(AT_BLOOD, "You pull back the lever, hold the bowcaster to your face and... death.", ch, nullptr, nullptr,
                TO_CHAR);
            act(AT_BLOOD, "$n blows his head off with a bowcaster quarrel!", ch, nullptr, nullptr, TO_ROOM);
            break;
        }
    }

    sprintf_s(logbuf, "%s has commited suicide.", ch->name);
    log_string(logbuf);
...
void do_bank(CHAR_DATA* ch, char* argument)
{
    ...
    else if (!str_prefix(arg1, "transfer"))
    {
        ...
        sprintf_s(logbuf, "%s transfers %ld credits to %s", ch->name, amount, victim->name);
        log_string(logbuf);
        ch->pcdata->bank -= amount;
...
void do_tune(CHAR_DATA* ch, char* argument)
{
    ...
    act(AT_WHITE, "You make some adjustments on the ships comm system.", ch, nullptr, nullptr, TO_CHAR);
    act(AT_WHITE, "$n makes some adjustments on the ships comm system.", ch, nullptr, nullptr, TO_ROOM);
    sprintf_s(buf, "Ships channel tuned to %s%d.\n\r", num == 0 ? "the public channel " : "", num);
    send_to_char(buf, ch);
    ship->channel = num;
    save_ship(ship);
}
void do_whisper(CHAR_DATA* ch, char* argument)
{
    ...
    sprintf_s(buf, "&B[%swhisper&B] &Cto %s: &W%s%s\n\r", color_str(AT_WHISPER, ch), PERS(victim, ch),
              color_str(AT_WHISPER, ch), arg2);
    send_to_char(buf, ch);
    sprintf_s(buf, "&B[%swhisper&B] &C%s whispers: &W%s%s\n\r", color_str(AT_WHISPER, victim), PERS(ch, victim),
              color_str(AT_WHISPER, victim), arg2);
...
void do_droptroops(CHAR_DATA* ch, char* argument)
{
    ...
    if ((pMobIndex = get_mob_index(82)) == nullptr)
        return;
...
void do_hale(CHAR_DATA* ch, char* argument)
{
    ...
    sprintf_s(buf, "&B[&CShip Hail&B] &C%s&B: &w%s", ship2->name, arg2);
    sprintf_s(buf1, "&B[&CShip Hail&B] &CYou send&B: &w%s", arg2);
    send_to_char(buf1, ch);
    sprintf_s(buf1, "&g%s adjusts some settings on the comm system and says:\n\r&C'&w%s&C'", ch->name, arg2);
    act(AT_GREEN, buf1, ch, nullptr, nullptr, TO_NOTVICT);
...
void do_rpconvert(CHAR_DATA* ch, char* argument)
{
    ...
    if (atoi(argument) == 1)
    {
        ...
        {
            if (UPPER(argument[0]) == UPPER(ability_name[iClass][0]) && !str_prefix(argument, ability_name[iClass]) &&
                str_prefix(argument, "force"))
            {
                ch->bonus[iClass] += 1;
                ch_printf(ch, "&GYour ability in %s has been increased by 1!\n\r", ability_name[iClass]);
                sprintf_s(buf, "%s increased class %s with rpconvert.", ch->name, ability_name[iClass]);
                log_string(buf);
    ...
    if (atoi(argument) == 3)
    {
        ...
        sprintf_s(buf, "%s increased %s by 5%% with rpconvert.\n\r", ch->name, skill_table[sn]->name);
        log_string(buf);
...
void do_arm(CHAR_DATA* ch, char* argument)
{
    ...
    {
        send_to_char("Usage: arm <timer> ###\n\r", ch);
        return;
    }
    if (!is_number(arg) || !is_number(argument))
    {
        send_to_char("The timer and password must be numbers\n\rSyntax: arm <timer> ###\n\r", ch);
        return;
    }
    ...
    if (timer < 1)
    {
        send_to_char("The timer must be set at least to 1.\n\r", ch);
        return;
    }

    if (timer > 10)
    {
        send_to_char("The timer must be lower than 10.\n\r", ch);
        return;
    }

    if (strlen(argument) != 3)
    {
        send_to_char("The password must be 3 digits.\n\r", ch);
        return;
    }
...
void do_disarmgrenade(CHAR_DATA* ch, char* argument)
{
    ...
    if (obj->timer <= 0)
    {
        send_to_char("It's not even armed yet\n\r", ch);
        return;
    }
    ...
    {
        ch_printf(ch, "&RIncorrect Password.\n\r");
        return;
    }
    obj->timer = 0;
    obj->value[5] = 0;
    ch_printf(ch, "You disarm %s.\n\r", obj->short_descr);
    act(AT_PLAIN, "$n disarms $p.", ch, obj, nullptr, TO_ROOM);
}
...
void do_invite(CHAR_DATA* ch, char* argument)
{
    ...
    {
        ch_printf(ch, "They don't want in!\n\r");
        return;
    }
    ...
    act(AT_ACTION, "$N gets invited into $n's room!", ch, nullptr, rch, TO_NOTVICT);
    return;
}
...
void do_buzz(CHAR_DATA* ch, char* argument)
{
    ...
    if ((pexit = get_exit(ch->in_room, get_dir(arg))) != nullptr)
    {
        ROOM_INDEX_DATA* to_room;
        EXIT_DATA* pexit_rev;

        if ((to_room = pexit->to_room) != nullptr && IS_SET(to_room->room_flags, ROOM_EMPTY_HOME))
        {
            send_to_char("No Need to use the intercom, nobody lives there!\n\r", ch);
            return;
        }
        else if ((to_room = pexit->to_room) != nullptr && !IS_SET(to_room->room_flags, ROOM_PLR_HOME))
        {
            send_to_char("Nobody Owns That Home!\n\r", ch);
            return;
        }
        std::string keyword = capitalize(dir_name[pexit->vdir]);
        act(AT_ACTION, "You use the intercom to the $d.", ch, nullptr, keyword.c_str(), TO_CHAR);
        act(AT_ACTION, "$n uses the intercom to the $d.", ch, nullptr, keyword.c_str(), TO_ROOM);
        if ((to_room = pexit->to_room) != nullptr && (pexit_rev = pexit->rexit) != nullptr &&
            pexit_rev->to_room == ch->in_room)
        {
            sprintf_s(buf, "%s uses the intercom from outside!\n\r", ch->name);
...
void do_debitorder(CHAR_DATA* ch, char* argument)
{
    ...
    {
        send_to_char("Syntax: debitorder <yes/no>\n\r", ch);
        send_to_char("You must agree to paying a one time fee of 1000 credits for the convience of a debit card!\n\r",
                     ch);
        return;
    }
    ...
    {
        send_to_char("That's fine, please do not let that hinder you from doing business with our bank!\n\r", ch);
        return;
    }
    ...
    else if (!strcmp(argument, "yes"))
    {
        ...
        {
            send_to_char("You just don't have enough money in your account to do that!\n\r", ch);
            return;
        }
        if ((ch->pcdata->bank - 1000) < 1000)
        {
            send_to_char(
                "The bank believes that it would be unreasonable for you to purchase a debit card at this time!\n\r",
                ch);
            return;
        }
        ...
        obj_to_char(obj, ch);
        ch->pcdata->bank -= 1000;
        send_to_char("You pay the $1000 to get your own debit card!\n\r", ch);
    }
    return;
}
