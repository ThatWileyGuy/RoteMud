 ...
 */
export bool check_skill(CHAR_DATA* ch, const char* command, char* argument)
{
    int sn;
...
export void learn_from_success(CHAR_DATA* ch, int sn)
{
...
export void learn_from_failure(CHAR_DATA* ch, int sn)
{
    int adept, gain, sklvl, learn, percent, chance;
    if (IS_NPC(ch) || ch->pcdata->learned[sn] == 0)
        return;
...
void do_gouge(CHAR_DATA* ch, char* argument)
{
    ...
    if (IS_NPC(ch) || percent < ch->pcdata->learned[gsn_gouge])
    {
        ...
        {
            act(AT_BLOOD, "Your fingers plunge into your victim's brain, causing immediate death!", ch, NULL, NULL,
                TO_CHAR);
        }
...
void do_search(CHAR_DATA* ch, char* argument)
{
    ...
    if (door != -1)
    {
        ...
        if ((pexit = get_exit(ch->in_room, door)) != NULL && IS_SET(pexit->exit_info, EX_SECRET) &&
...
void do_dualstab(CHAR_DATA* ch, char* argument)
{
    ...
    if ((obj = get_eq_char(ch, WEAR_WIELD)) == NULL || (obj->value[3] != WEAPON_VIBRO_BLADE) ||
        (obj = get_eq_char(ch, WEAR_DUAL_WIELD)) == NULL || (obj->value[3] != WEAPON_VIBRO_BLADE))
    {
 ...
 */
export void disarm(CHAR_DATA* ch, CHAR_DATA* victim)
{
    OBJ_DATA *obj, *tmpobj;
 ...
 * Caller must check for successful attack.
 */
export void trip(CHAR_DATA* ch, CHAR_DATA* victim)
{
 ...
 * Check for parry.
 */
export bool check_parry(CHAR_DATA* ch, CHAR_DATA* victim)
{
    int chances;
    OBJ_DATA* wield;
    FORCE_SKILL* fskill;
 ...
 * Check for dodge.
 */
export bool check_dodge(CHAR_DATA* ch, CHAR_DATA* victim)
{
    int chances;
...
export bool check_grip(CHAR_DATA* ch, CHAR_DATA* victim)
{
    int chance;
