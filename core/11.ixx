...
export std::string beam_name(sh_int type, bool plural)
{
...
export std::string primary_beam_name(SHIP_DATA* ship)
{
...
export std::string secondary_beam_name(SHIP_DATA* ship)
{
...
void do_makegoggles(CHAR_DATA* ch, char* argument)
{
    ...
    default:
        ...
        chance = IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->learned[gsn_makegoggles]);
...
void do_makemissile(CHAR_DATA* ch, char* argument)
{
    ...
    default:
        ...
        chance = IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->learned[gsn_makemissile]);
        if (number_percent() < chance)
        {
    ...
    if (number_percent() > chance * 2 || (!checktool) || (!checkdura) || (!checkbatt) || (!checkcirc))
    {
                ...
                if (IS_SET(pexit->exit_info, EX_CLOSED)) // Door in the way?
                {
                    if (!IS_SET(pexit->exit_info, EX_BASHPROOF))
                    {
                        ...
                        for (rch = ch->in_room->first_person; rch; rch = rch->next_in_room)
                        {
                            if (rch == ch)
                                continue;
                            sprintf_s(buf, "%s launches a missile %s, and it EXPLODES upon impact of the doorway!",
                                      PERS(ch, rch), dtxt);
                    ...
                    else // is bashproof
                    {
                        for (rch = ch->in_room->first_person; rch; rch = rch->next_in_room)
                        {
                            if (rch == ch)
                                continue;
                            sprintf_s(buf, "&w%s fires a missile %s, and it explodes upon impact of the doorway!",
                                      PERS(ch, rch), dtxt);
                            send_to_char(buf, rch);
                        }
                        send_to_char("&RThe missile flies directly into the doorway, and EXPLODES!\n\r", ch);
                ...
                else if (number_range(-15, chance + (rch->perm_lck - 12)) >= -5) // Miss, but still damages
                {
                    ...
                    for (zch = proom->first_person; zch; zch = zch->next_in_room)
                    {
                        if (zch == rch)
                            continue;
                        ch_printf(zch,
                                  "%s barely dodges a missile fired at them from %s, but it still explodes nearby!\n\r",
                                  PERS(rch, zch), ftxt);
                    }
                    if (wield->value[1] == 1)
                        explode_emissile(ch, proom, wield->value[3], wield->value[4], 1);
                ...
                if (IS_SET(pexit->exit_info, EX_CLOSED)) // Door in the way?
                {
                    if (!IS_SET(pexit->exit_info, EX_BASHPROOF))
                    {
                        ...
                        for (rch = proom->first_person; rch; rch = rch->next_in_room)
                        {
                            ch_printf(
                                rch,
                                "&wA missile flies in from the %s, and explodes upon impact of the doorway %s!\n\r",
                                ftxt, dtxt);
                            ch_printf(rch, "&RThe doorway is reduced to nothing but debris.&w\n\r");
                            ch_printf(ch,
                                      "&WThe missile travels %d room%s away, and hits a door, which is destroyed!\n\r",
                                      dist, dist > 1 ? "s" : "");
                        }
                    }
                    else
                    {
                        for (rch = proom->first_person; rch; rch = rch->next_in_room)
                        {
                            ...
                            ch_printf(ch,
                                      "&WThe missile travels %d room%s, and hits a door, which remains undamaged!\n\r",
                                      dist, dist > 1 ? "s" : "");
                        }
                    }
                    if (wield->value[1] == 1)
                    ...
                    for (rch = proom->first_person; rch; rch = rch->next_in_room)
                    {
                        if (number_range(1, 4) == 4)
                        {
                            ch_printf(
                                rch,
                                "&wYou are too close to the door, and the EXPLOSION knocks you to the ground!&w\n\r");
                            for (zch = proom->first_person; zch; zch = zch->next_in_room)
                            {
                                if (zch == rch)
                                    continue;
                                ch_printf(zch,
                                          "&w%s is too close to the door, and the EXPLOSION knocks them to the ground!",
                                          PERS(zch, rch));
                            }
                            rch->position = POS_SITTING;
                        }
...
void do_link(CHAR_DATA* ch, char* argument)
{
    ...
    if (!str_prefix(argument, "launchers"))
    {
        ...
        if ((ship->maxmissiles && ship->maxtorpedos) || (ship->maxmissiles && ship->maxrockets) ||
