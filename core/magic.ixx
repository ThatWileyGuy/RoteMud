...
int ch_slookup(CHAR_DATA* ch, const char* name)
{
    ...
    for (sn = 0; sn < top_sn; sn++)
    {
        if (!skill_table[sn]->name)
            break;
        if (ch->pcdata->learned[sn] > 0 && LOWER(name[0]) == LOWER(skill_table[sn]->name[0]) &&
 ...
 */
export int herb_lookup(const char* name)
{
    int sn;
 ...
 */
export int skill_lookup(const char* name)
{
    int sn;
 ...
 */
export SKILL_TYPE* get_skilltype(int sn)
{
    if (sn >= TYPE_PERSONAL)
        return nullptr;
    if (sn >= TYPE_HERB)
 ...
 */
export int bsearch_skill_exact(const char* name, int first, int top)
{
    int sn;
 ...
 */
export int slot_lookup(int slot)
{
    extern bool fBootDb;
    int sn;
 ...
 */
export void failed_casting(SKILL_TYPE* skill, CHAR_DATA* ch, CHAR_DATA* victim, OBJ_DATA* obj)
{
    sh_int chitroom = (skill->type == SKILL_SPELL ? AT_MAGIC : AT_ACTION);
    sh_int chit = (skill->type == SKILL_SPELL ? AT_MAGIC : AT_HIT);
    sh_int chitme = (skill->type == SKILL_SPELL ? AT_MAGIC : AT_HITME);
 ...
 */
export int ris_save(CHAR_DATA* ch, int chance, int ris)
{
    sh_int modifier;
 ...
 */
export bool saves_poison_death(int level, CHAR_DATA* victim)
{
    int save;
    if (!victim)
    {
...
export bool saves_para_petri(int level, CHAR_DATA* victim)
{
    int save;
...
export bool saves_spell_staff(int level, CHAR_DATA* victim)
{
    int save;
...
void do_cast(CHAR_DATA* ch, char* argument)
{
    ...
    default:
        ...
        sprintf_s(staticbuf, "%s %s", arg2, target_name);
    ...
    case 1:
        ...
        if (!ch->dest_buf || !IS_VALID_SN(sn) || skill->type != SKILL_SPELL)
        {
            ...
            bug("do_cast: ch->dest_buf nullptr or bad sn (%d)", sn);
            return;
        }
        mana = IS_NPC(ch) ? 0 : skill->min_mana;
        ...
        if (skill->participants > 1)
        {
            ...
            for (tmp = ch->in_room->first_person; tmp; tmp = tmp->next_in_room)
                if (tmp != ch && (t = get_timerptr(tmp, TIMER_DO_FUN)) != nullptr && t->count >= 1 &&
            ...
            if (cnt >= skill->participants)
            {
                for (tmp = ch->in_room->first_person; tmp; tmp = tmp->next_in_room)
                    ...
                    {
                        extract_timer(tmp, t);
                        act(AT_MAGIC, "Channeling your energy into $n, you help direct the force", ch, nullptr, tmp,
                            TO_VICT);
    ...
    if (skill->target == TAR_CHAR_OFFENSIVE && victim && !char_died(victim) &&
        !IS_SET(victim->affected_by, AFF_PARALYSIS) && victim != ch)
    {
 ...
 */
export ch_ret obj_cast_spell(int sn, int level, CHAR_DATA* ch, CHAR_DATA* victim, OBJ_DATA* obj)
{
    void* vo;
...
ch_ret spell_charm_person(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    sprintf_s(buf, "%s has charmed %s.", ch->name, victim->name);
...
ch_ret spell_chill_touch(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    static const sh_int dam_each[] = {3,  4,  5,  6,  7,  8,  9,  12, 13, 13, 13, 14, 14, 14, 15, 15, 15, 16,
                                      ...
                                      29, 29, 30, 30, 31, 31, 32, 32, 33, 34, 34, 35, 35, 36, 37, 37, 38};
    AFFECT_DATA af;
    int dam;
...
ch_ret spell_colour_spray(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    static const sh_int dam_each[] = {3,  4,  5,  6,  7,  8,  9,  10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 55,
                                      ...
                                      82, 82, 83, 83, 84, 85, 85, 86, 86, 87, 88, 88, 89, 89, 90, 91, 91};
    int dam;
...
ch_ret spell_create_water(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (water > 0)
    {
        ...
        if (!is_name("water", obj->name))
        {
            ...
            sprintf_s(buf, "%s water", obj->name);
            STRFREE(obj->name);
...
ch_ret spell_dispel_magic(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    else if (victim->affected_by == race_table[victim->race].affected || level < victim->top_level ||
...
ch_ret spell_fireball(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    static const sh_int dam_each[] = {1,  1,  4,  7,  10, 13, 16, 19, 22, 25, 28, 31, 34, 37, 40, 40, 41, 42,
                                      ...
                                      66, 66, 67, 68, 68, 69, 69, 70, 71, 71, 72, 72, 73, 73, 74, 75, 75};
    int dam;
...
ch_ret spell_force_disarm(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    {
        act(AT_BLUE, "You hold your hand out using the force to call $N's weapon to you but $S grip is to tight.", ch,
            nullptr, victim, TO_CHAR);
        act(AT_BLUE, "$n holds $s hand out using the force to call your weapon to $m but your grip is to tight.", ch,
            nullptr, victim, TO_VICT);
        act(AT_BLUE, "$n holds $s hand out using the force to call $N's weapon to $m but $N's grip is to tight.", ch,
    ...
    act(AT_BLUE, "You hold your hand out using the force to call $N's weapon to you.", ch, nullptr, victim, TO_CHAR);
    act(AT_BLUE, "$n holds $s hand out using the force to call your weapon to $m.", ch, nullptr, victim, TO_VICT);
    act(AT_BLUE, "$n holds $s hand out using the force to call $N's weapon to $m.", ch, nullptr, victim, TO_NOTVICT);
    obj_from_char(obj);
    obj_to_char(obj, ch);
    return rNONE;
}
...
ch_ret spell_injure(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if ((number_percent() <= (chance - 80)))
    {
        ...
        act(AT_WHITE, "$N falls to the ground, unconcious.", ch, nullptr, victim, TO_ROOM);
...
ch_ret spell_identify(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    else if ((victim = get_char_room(ch, target_name)) != nullptr)
    {
        ...
        ch_printf(ch, "%s looks like %s.\n\r", victim->name, aoran(get_race(victim)).c_str());
...
ch_ret spell_locate_object(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    for (obj = first_object; obj; obj = obj->next)
    {
        ...
        if (cnt >= MAX_NEST)
        {
            sprintf_s(buf, "spell_locate_obj: object [%d] %s is nested more than %d times!", obj->pIndexData->vnum,
...
ch_ret spell_sleep(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (IS_AFFECTED(victim, AFF_SLEEP) || (chance = ris_save(victim, tmp, RIS_SLEEP)) == 1000 ||
        level < victim->top_level || (victim != ch && IS_SET(victim->in_room->room_flags, ROOM_SAFE)) ||
    ...
    if (!IS_NPC(victim))
    {
        sprintf_s(log_buf, "%s has cast sleep on %s.", ch->name, victim->name);
...
ch_ret spell_ventriloquate(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    sprintf_s(buf1, "%s says '%s'.\n\r", speaker, target_name);
    sprintf_s(buf2, "Someone makes %s say '%s'.\n\r", speaker, target_name);
    buf1[0] = UPPER(buf1[0]);
...
ch_ret spell_farsight(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    */
    if ((victim = get_char_world(ch, target_name)) == nullptr || victim == ch || !victim->in_room ||
        IS_SET(victim->in_room->room_flags, ROOM_PRIVATE) || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY) ||
        victim->top_level >= level + 15 || (IS_NPC(victim) && IS_SET(victim->act, ACT_PROTOTYPE)) ||
...
ch_ret spell_animate_dead(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if ((pMobIndex = get_mob_index((int)abs(corpse->cost))) == nullptr)
    {
    ...
    if (IS_IMMORTAL(ch) || (chance(ch, 75) && pMobIndex->level - ch->top_level < 10))
    {
        ...
        mob->max_hit =
            UMAX(URANGE(mob->max_hit / 4, (mob->max_hit * corpse->value[3]) / 100, ch->top_level * dice(20, 10)), 1);
        ...
        sprintf_s(buf, "animated corpse %s", pMobIndex->player_name);
        STRFREE(mob->name);
        mob->name = STRALLOC(buf);

        sprintf_s(buf, "The animated corpse of %s", pMobIndex->short_descr);
        STRFREE(mob->short_descr);
        mob->short_descr = STRALLOC(buf);

        sprintf_s(buf, "An animated corpse of %s struggles with the horror of its undeath.\n\r",
...
ch_ret spell_possess(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    sprintf_s(buf, "You have possessed %s!\n\r", victim->short_descr);
...
ch_ret spell_spiral_blast(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    for (vch = ch->in_room->first_person; vch; vch = vch_next)
    {
        ...
        {
            act(AT_MAGIC,
                "Swirling colours radiate from $n"
                ", encompassing $N.",
                ch, ch, vch, TO_ROOM);
            act(AT_MAGIC,
                "Swirling colours radiate from you,"
                " encompassing $N",
                ch, ch, vch, TO_CHAR);
...
ch_ret spell_affect(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    for (; victim; victim = victim->next_in_room)
    {
        if (groupsp || areasp)
        {
            if ((groupsp && !is_same_group(victim, ch)) || IS_SET(victim->immune, RIS_MAGIC) ||
...
ch_ret spell_create_mob(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    mob->max_hit =
        mob->top_level * 8 + number_range(mob->top_level * mob->top_level / 4, mob->top_level * mob->top_level);
    mob->hit = mob->max_hit;
    mob->gold = 0;
...
ch_ret spell_smaug(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    case TAR_IGNORE:
        ...
        if (SPELL_FLAG(skill, SF_AREA) && ((SPELL_ACTION(skill) == SA_DESTROY && SPELL_CLASS(skill) == SC_LIFE) ||
...
ch_ret spell_ethereal_fist(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(AT_MAGIC, "A fist of black, otherworldly ether rams into $N, leaving $M looking stunned!", ch, nullptr, victim,
        TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_disruption(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam = 0;
    act(AT_MAGIC, "A weird energy encompasses $N, causing you to question $S continued existence.", ch, nullptr, victim,
        TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_acetum_primus(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam = 3 * dam / 4;
    act(AT_MAGIC, "A cloak of primal acid enshrouds $N, sparks form as it consumes all it touches. ", ch, nullptr, victim,
        TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_galvanic_whip(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(AT_MAGIC, "$n conjures a whip of ionized particles, which lashes ferociously at $N.", ch, nullptr, victim,
        TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_quantum_spike(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(AT_MAGIC, "$N seems to dissolve into tiny unconnected particles, then is painfully reassembled.", ch, nullptr,
        victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_steal_life(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(AT_BLUE, "You drain $N's life restoring some of your own.", ch, nullptr, victim, TO_CHAR);
    act(AT_BLUE, "Your life force seems to flow from your body.", ch, nullptr, victim, TO_VICT);
    act(AT_BLUE, "$n's body seems to glow with new found life.", ch, nullptr, victim, TO_NOTVICT);
...
ch_ret spell_calm(int sn, int level, CHAR_DATA* ch, void* vo)
{
    CHAR_DATA* vch;
    CHAR_DATA* victim = (CHAR_DATA*)vo;
    if ((vch = who_fighting(victim)) == nullptr)
    {
        send_to_char("They aren't fighting anyone.\n\r", ch);
        return rSPELL_FAILED;
    }
    act(AT_BLUE, "You calm $N down, causing them to loose interest in your fight.", ch, nullptr, victim, TO_CHAR);
    act(AT_BLUE, "You feel yourself calm down, loosing interest in the fight.", ch, nullptr, victim, TO_VICT);
    act(AT_BLUE, "$N seems to loose interest in $S fight with $n.", ch, nullptr, victim, TO_NOTVICT);
    stop_fighting(victim, true);
...
ch_ret spell_forcepush(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(AT_BLUE,
        "You extend your arm in $N's direction and send $M flying into a near by wall with a push of the force.", ch,
        nullptr, victim, TO_CHAR);
    act(AT_BLUE, "$n extends $s hand towards you and you feel yourself sent tumbling into a nearby wall.", ch, nullptr,
        victim, TO_VICT);
...
ch_ret spell_black_lightning(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(AT_BLUE, "Bolts of electricity shoot from the fingers of $n, sending $N into a fit of painful spasms.", ch,
        nullptr, victim, TO_NOTVICT);
    act(AT_BLUE, "Bolts of electricity shoot from your fingertips, sending $N into a fit of painful spasms.", ch, nullptr,
        victim, TO_CHAR);
    act(AT_BLUE, "Intense pain spreads through your body as bolts of electricity from $N assault you.", victim, nullptr,
        ch, TO_CHAR);
...
ch_ret spell_midas_touch(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (!CAN_WEAR(obj, ITEM_TAKE) || (obj->item_type == ITEM_CORPSE_NPC) || (obj->item_type == ITEM_DROID_CORPSE) ||
