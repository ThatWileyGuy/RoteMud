...
static BFS_QUEUE_ENTRY *queue_head = nullptr, *queue_tail = nullptr, *room_queue = nullptr;
...
void do_track(CHAR_DATA* ch, char* argument)
{
    ...
    case BFS_ALREADY_THERE:
        ...
        break;
    case BFS_NO_PATH:
        sprintf_s(buf, "You can't sense a trail from here.\n\r");
        send_to_char(buf, ch);
...
export void found_prey(CHAR_DATA* ch, CHAR_DATA* victim)
{
    char buf[MAX_STRING_LENGTH];
    ...
    sprintf_s(victname, IS_NPC(victim) ? victim->short_descr : race_table[victim->race].race_name);
    ...
    if (!can_see(ch, victim))
    {
        ...
        {
        case 0:
            sprintf_s(buf, "Don't make me find you, %s!", victname);
            do_say(ch, buf);
            break;
        case 1:
            ...
            sprintf_s(buf, "I can smell your blood!");
            do_say(ch, buf);
            break;
        case 2:
            sprintf_s(buf, "I'm going to tear %s apart!", victname);
            do_yell(ch, buf);
            break;
        case 3:
    ...
    if (IS_SET(ch->in_room->room_flags, ROOM_SAFE))
    {
        ...
        case 0:
            ...
            break;
        case 1:
            sprintf_s(buf, "Let's take this outside, %s", victname);
            do_say(ch, buf);
            break;
        case 2:
            sprintf_s(buf, "%s is a yellow-bellied wimp!", victname);
            do_yell(ch, buf);
            break;
        case 3:
    ...
    {
    case 0:
        sprintf_s(buf, "Your blood is mine, %s!", victname);
        do_yell(ch, buf);
        break;
    case 1:
        sprintf_s(buf, "Alas, we meet again, %s!", victname);
        do_say(ch, buf);
        break;
    case 2:
        sprintf_s(buf, "What do you want on your tombstone, %s?", victname);
        do_say(ch, buf);
        break;
    case 3:
...
export void hunt_victim(CHAR_DATA* ch)
{
    bool found;
    CHAR_DATA* tmp;
    sh_int ret;
    ...
    if (ret == BFS_NO_PATH)
    {
        ...
        {
            ret = number_door();
            if ((pexit = get_exit(ch->in_room, ret)) == nullptr || !pexit->to_room ||
    ...
    else
    {
        ...
        if (!ch->hunting)
        {
            ...
            {
                char buf[MAX_STRING_LENGTH];
                sprintf_s(buf, "Hunt_victim: no ch->in_room!  Mob #%d, name: %s.  Placing mob in limbo.",
