...
export void reset_colors(CHAR_DATA* ch)
{
    memcpy(&ch->colors, &default_set, sizeof(default_set));
}

export const char* color_str(sh_int AType, CHAR_DATA* ch)
{
    if (!ch)
    {
...
/* Moved from comm.c */
export void set_char_color(sh_int AType, CHAR_DATA* ch)
{
    if (!ch || !ch->desc)
        return;
...
export void set_pager_color(sh_int AType, CHAR_DATA* ch)
{
    if (!ch || !ch->desc)
        return;
 ...
 */
export void send_to_char_color(const char* txt, CHAR_DATA* ch)
{
    const char* colstr;
    const char* prevstr = txt;
    char colbuf[20];
...
export void send_to_pager_color(const char* txt, CHAR_DATA* ch)
{
    const char* colstr;
    const char* prevstr = txt;
    char colbuf[20];
...
export void send_to_char(const char* txt, CHAR_DATA* ch)
{
    send_to_char_color(txt, ch);
    return;
}

export void send_to_pager(const char* txt, CHAR_DATA* ch)
{
    send_to_pager_color(txt, ch);
    return;
}
// newchprintf
export void ch_printf(CHAR_DATA* ch, const char* fmt, ...)
{
...
export void pager_printf(CHAR_DATA* ch, const char* fmt, ...)
{
    char buf[MAX_STRING_LENGTH * 2];
    va_list args;
