...
char* drawlife(int min, int max)
{
    ...
    if (per > 100)
        per = 100;
    if (per == 100)
        sprintf_s(buf, "&G&W[&R|||||&Y||||&G||||&W]");
    else if (per >= 90 && per < 100)
        sprintf_s(buf, "&G&W[&R|||||&Y||||&G|||&G-&W]");
...
char* drawalign(int align)
{
    static char buf[MAX_STRING_LENGTH];
    if (align >= 100)
        sprintf_s(buf, "&W[&C============&W|&W]");
    else if (align >= 90 && align < 100)
        sprintf_s(buf, "&W[&C===========&W|&C=&W]");
...
void do_score(CHAR_DATA* ch, char* argument)
{
    ...
    ch_printf(ch, "&z|  &cWeight:  %s        &cItems: %s  &z|&W\n\r",
              drawlife(victim->carry_weight, can_carry_w(victim)), drawlife(victim->carry_number, can_carry_n(victim)));
    send_to_char("&z|----------------------------------------------------------|&W\n\r", ch);
...
void do_oldscore(CHAR_DATA* ch, char* argument)
{
    ...
    set_char_color(AT_OLDSCORE, ch);
    ch_printf(ch, "You are %s%s&w&W%s, level %d, %d years old (%d hours).\n\r", ch->name,
              IS_NPC(ch) ? "" : ch->pcdata->title, color_str(AT_OLDSCORE, ch), ch->top_level, get_age(ch),
              (get_age(ch) - 17));
    ...
    ch_printf(ch, "You are carrying %d/%d items with weight %d/%d kg.\n\r", ch->carry_number, can_carry_n(ch),
              ch->carry_weight, can_carry_w(ch));

    ch_printf(ch, "Str: %d  Int: %d  Wis: %d  Dex: %d  Con: %d  Cha: %d  Lck: ??  Frc: ??\n\r", get_curr_str(ch),
...
export void set_title(CHAR_DATA* ch, char* title)
{
    char buf[MAX_STRING_LENGTH];
...
void do_title(CHAR_DATA* ch, char* argument)
{
    ...
    if ((get_trust(ch) <= LEVEL_IMMORTAL) && (!nifty_is_name(ch->name, remand(argument).c_str())))
    {
...
void do_report(CHAR_DATA* ch, char* argument)
{
    ...
    ch_printf(ch, "%sYou report: %d/%d hp %d/%d mv.\n\r", color_str(AT_REPORT, ch), ch->hit, ch->max_hit, ch->move,
              ch->max_move);
