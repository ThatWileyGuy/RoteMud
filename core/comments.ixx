...
void do_comment(CHAR_DATA* ch, char* argument)
{
    ...
    if (!str_cmp(arg, "read"))
    {
        ...
        for (pnote = victim->comments; pnote; pnote = pnote->next)
        {
            ...
            if (vnum == anum || fAll)
            {
                sprintf_s(buf, "[%3d] %s: %s\n\r%s\n\rTo: %s\n\r", vnum, pnote->sender, pnote->subject, pnote->date,
                          pnote->to_list);
                send_to_char(buf, ch);
    ...
    if (!str_cmp(arg, "post"))
    {
        ...
        {
            fprintf(fp, "Sender  %s~\nDate    %s~\nTo      %s~\nSubject %s~\nText\n%s~\n\n", pnote->sender, pnote->date,
                    pnote->to_list, pnote->subject, pnote->text);
            fclose(fp);
        }
#endif
...
export void fwrite_comments(CHAR_DATA* ch, FILE* fp)
{
    NOTE_DATA* pnote;
...
export void fread_comment(CHAR_DATA* ch, FILE* fp)
{
    NOTE_DATA* pnote;
