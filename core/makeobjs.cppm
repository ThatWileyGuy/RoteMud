 ...
 * Make a trap.
 */
export OBJ_DATA* make_trap(int v0, int v1, int v2, int v3)
{
    OBJ_DATA* trap;
 ...
 */
export void make_scraps(OBJ_DATA* obj)
{
    ...
    {
        sprintf_s(buf, scraps->short_descr, obj->short_descr);
        STRFREE(scraps->short_descr);
        scraps->short_descr = STRALLOC(buf);
        if (obj->item_type != ITEM_SHIPBOMB)
        {
            sprintf_s(buf, scraps->description, obj->short_descr);
            STRFREE(scraps->description);
            scraps->description = STRALLOC(buf);
        }
        else
        {
            sprintf_s(buf, "Bits of debris, buildings, people, and bomb scraps litter the area.");
            STRFREE(scraps->description);
            scraps->description = STRALLOC(buf);
 ...
 */
export void make_corpse(CHAR_DATA* ch, char* killer)
{
    char buf[MAX_STRING_LENGTH];
    OBJ_DATA* corpse;
    OBJ_DATA* obj;
    ...
    sprintf_s(buf, "corpse %s", name);
    STRFREE(corpse->name);
    corpse->name = STRALLOC(buf);

    sprintf_s(buf, corpse->short_descr, name);
    STRFREE(corpse->short_descr);
    corpse->short_descr = STRALLOC(buf);

    sprintf_s(buf, corpse->description, name);
...
export void make_bloodstain(CHAR_DATA* ch)
{
    OBJ_DATA* obj;
 ...
 * make some coinage
 */
export OBJ_DATA* create_money(int amount)
{
    char buf[MAX_STRING_LENGTH];
    OBJ_DATA* obj;
