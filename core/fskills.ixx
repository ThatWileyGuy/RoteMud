...
void fskill_finfo(CHAR_DATA* ch, char* argument)
{
    ...
    ch_printf(ch, "   Mana: %s%d%s/%s%d    ", ch->force_type == FORCE_JEDI ? "&B" : "&R", ch->mana,
    ...
    case 1:
        ...
        if (force_calc > 0 && victim->force_identified == 0)
        {
            ...
            if (x < 150)
                ...
                send_to_char(
                    force_parse_string(
                        ch, victim, "You feel briefly lightheaded...the force possesses this one like few others..."),
                    ch);
        }
...
void fskill_promote(CHAR_DATA* ch, char* argument)
{
    ...
    /*
        ...
        {
            send_to_char(force_parse_string(ch,victim,"$n cannot be promoted until the Jedi council is
       finished!\r\n"),ch); return;
        }
    */
    ...
    force_send_to_room(ch, victim, force_parse_string(ch, victim, fskill->room_effect[0]));
    victim->max_mana =
        (victim->force_control + victim->force_sense + victim->force_alter) * 3 * victim->force_level_status;
...
void secondslash(CHAR_DATA* ch, CHAR_DATA* victim)
{
    ...
    dam = URANGE(
        40,
        number_range(0, ch->force_skill[FORCE_SKILL_STRIKE] * ch->force_control / (20 * (4 - ch->force_level_status))),
        100);
    if (IS_AFFECTED(victim, AFF_SANCTUARY))
        dam /= 2;
    victim->hit -= dam / 2;
...
void fskill_whirlwind(CHAR_DATA* ch, char* argument)
{
    ...
    force_send_to_room(ch, victim, force_parse_string(ch, victim, fskill->room_effect[1]));
    dam = URANGE(40,
                 number_range(0, ch->force_skill[FORCE_SKILL_WHIRLWIND] * ch->force_control /
                                     (20 * (4 - ch->force_level_status))),
                 100);
    if (IS_AFFECTED(victim, AFF_SANCTUARY))
        dam /= 2;
...
void fskill_squeeze(CHAR_DATA* ch, char* argument)
{
    ...
    case 1:
        if (number_range(0, 4) != 0 &&
            (number_range(0, 100) > ch->force_skill[FORCE_SKILL_SQUEEZE] || number_range(0, 100) > ch->force_control))
        {
            send_to_char(force_parse_string(ch, victim, fskill->ch_effect[2]), ch);
            send_to_char(force_parse_string(ch, victim, fskill->victim_effect[2]), victim);
            force_send_to_room(ch, victim, force_parse_string(ch, victim, fskill->room_effect[2]));
...
void fskill_force_lightning(CHAR_DATA* ch, char* argument)
{
    ...
    case 1:
        ...
        send_to_char(force_parse_string(ch, victim, fskill->victim_effect[1]), victim);
        force_send_to_room(ch, victim, force_parse_string(ch, victim, fskill->room_effect[1]));
        victim->hit -= number_range(
            50, ((ch->force_alter * 200 / 100) + (ch->force_sense * 200 / 100) + (ch->force_control * 200 / 100)));
        force_learn_from_success(ch, fskill);
        if (!ch->dest_buf)
            break;
        DISPOSE(ch->dest_buf);
...
void fskill_makelightsaber(CHAR_DATA* ch, char* argument)
{
    ...
    default:
        ...
        chance = IS_NPC(ch) ? ch->top_level : (int)(ch->force_skill[FORCE_SKILL_MAKELIGHTSABER]);
...

    {
        ...
        //         exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) ); gain_exp(ch, xpgain, FORCE_ABILITY); ch_printf(
        //         ch , "You gain %d force experience.", xpgain );
    }
...
void fskill_makedualsaber(CHAR_DATA* ch, char* argument)
{
    ...
    default:
        ...
        chance = IS_NPC(ch) ? ch->top_level : (int)(ch->force_skill[FORCE_SKILL_MAKEDUALSABER]);
        if (number_percent() < chance)
        {
...
void fskill_finish(CHAR_DATA* ch, char* argument)
{
    ...
    */
    ...
    {
        sprintf_s(buf, "%s killed by %s at %d, using finish.\n\r", victim->name, ch->name, ch->in_room->vnum);
        log_string(buf);
        raw_kill(ch, victim);
...
export bool check_reflect(CHAR_DATA* ch, CHAR_DATA* victim, int dam)
{
            ...
            if (rch)
            {
                if (IS_NPC(rch))
                {
                    ...
                    else
                    {
                        ...
                        sprintf_s(roombuf, "%s hits %s with a reflected shot.\r\n", victim->name, rch->short_descr);
