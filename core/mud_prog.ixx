 ...
 */
export CHAR_DATA* supermob;
ACT_PROG_DATA* room_act_list;
export ACT_PROG_DATA* obj_act_list;
export ACT_PROG_DATA* mob_act_list;

/*
 * Global variables to handle sleeping mud progs.
 */
MPSLEEP_DATA* first_mpsleep = nullptr;
MPSLEEP_DATA* last_mpsleep = nullptr;
...
bool mprog_seval(const char* lhs, const char* opr, const char* rhs, CHAR_DATA* mob)
{
    ...
    sprintf_s(log_buf, "Improper MOBprog operator '%s'", opr);
    progbug(log_buf, mob);
    return 0;
}
    ...
    *pchck = '\0';
    ...
    if (cvar[0] == '$')
    {
        ...
        case 'p':
            ...
            break;
        default:
            sprintf_s(rval, "Bad argument '%c' to '%s'", cvar[0], chck);
            progbug(rval, mob);
            return BERR;
        }
        if (!chkchar && !chkobj)
    ...
    if (!str_cmp(chck, "economy"))
    {
        ...
        if (!idx)
        {
            if (!mob->in_room)
            {
                progbug("'economy' ifcheck: mob in nullptr room with no room vnum "
                        "argument",
                        mob);
                return BERR;
            }
        ...
        return mprog_veval(((room->area->high_economy > 0) ? 1000000000 : 0) + room->area->low_economy, opr, atoi(rval),
                           mob);
    }
    ...
    *t = '\0';
    ...
    case 'i':
        if (mob && !char_died(mob))
        {
            ...
            strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        break;
    ...
    case 'I':
        if (mob && !char_died(mob))
        {
            if (mob->short_descr)
            {
                strcpy_s(t, MAX_INPUT_LENGTH, mob->short_descr);
            }
            else
            {
                strcpy_s(t, MAX_INPUT_LENGTH, "someone");
    ...
    case 'N':
        if (actor && !char_died(actor))
        {
            if (can_see(mob, actor))
                if (IS_NPC(actor))
                    strcpy_s(t, MAX_INPUT_LENGTH, actor->short_descr);
                else
                {
                    strcpy_s(t, MAX_INPUT_LENGTH, actor->name);
                    strcat_s(t, MAX_INPUT_LENGTH, actor->pcdata->title);
                }
            else
                strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
    ...
    case 'T':
        if (vict && !char_died(vict))
        {
            if (can_see(mob, vict))
                if (IS_NPC(vict))
                    strcpy_s(t, MAX_INPUT_LENGTH, vict->short_descr);
                else
                {
                    strcpy_s(t, MAX_INPUT_LENGTH, vict->name);
                    strcat_s(t, MAX_INPUT_LENGTH, " ");
                    strcat_s(t, MAX_INPUT_LENGTH, vict->pcdata->title);
                }
            else
                strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
    ...
    case 'R':
        if (rndm && !char_died(rndm))
        {
            if (can_see(mob, rndm))
                if (IS_NPC(rndm))
                    strcpy_s(t, MAX_INPUT_LENGTH, rndm->short_descr);
                else
                {
                    strcpy_s(t, MAX_INPUT_LENGTH, rndm->name);
                    strcat_s(t, MAX_INPUT_LENGTH, " ");
                    strcat_s(t, MAX_INPUT_LENGTH, rndm->pcdata->title);
                }
            else
                strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
    ...
    case 'e':
        if (actor && !char_died(actor))
        {
            can_see(mob, actor) ? strcpy_s(t, MAX_INPUT_LENGTH, he_she[actor->sex])
                                : strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "it");
        break;

    case 'm':
        if (actor && !char_died(actor))
        {
            can_see(mob, actor) ? strcpy_s(t, MAX_INPUT_LENGTH, him_her[actor->sex])
                                : strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "it");
    ...
    case 's':
        if (actor && !char_died(actor))
        {
            can_see(mob, actor) ? strcpy_s(t, MAX_INPUT_LENGTH, his_her[actor->sex])
                                : strcpy_s(t, MAX_INPUT_LENGTH, "someone's");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "its'");
        break;

    case 'E':
        if (vict && !char_died(vict))
        {
            can_see(mob, vict) ? strcpy_s(t, MAX_INPUT_LENGTH, he_she[vict->sex])
                               : strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "it");
    ...
    case 'M':
        if (vict && !char_died(vict))
        {
            can_see(mob, vict) ? strcpy_s(t, MAX_INPUT_LENGTH, him_her[vict->sex])
                               : strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "it");
    ...
    case 'S':
        if (vict && !char_died(vict))
        {
            can_see(mob, vict) ? strcpy_s(t, MAX_INPUT_LENGTH, his_her[vict->sex])
                               : strcpy_s(t, MAX_INPUT_LENGTH, "someone's");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "its'");
    ...
    case 'j':
        if (mob && !char_died(mob))
        {
            strcpy_s(t, MAX_INPUT_LENGTH, he_she[mob->sex]);
        }
        else
        {
            strcpy_s(t, MAX_INPUT_LENGTH, "it");
    ...
    case 'k':
        if (mob && !char_died(mob))
        {
            strcpy_s(t, MAX_INPUT_LENGTH, him_her[mob->sex]);
        }
        else
        {
            strcpy_s(t, MAX_INPUT_LENGTH, "it");
    ...
    case 'l':
        if (mob && !char_died(mob))
        {
            strcpy_s(t, MAX_INPUT_LENGTH, his_her[mob->sex]);
        }
        else
        {
            strcpy_s(t, MAX_INPUT_LENGTH, "it");
    ...
    case 'J':
        if (rndm && !char_died(rndm))
        {
            can_see(mob, rndm) ? strcpy_s(t, MAX_INPUT_LENGTH, he_she[rndm->sex])
                               : strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "it");
    ...
    case 'K':
        if (rndm && !char_died(rndm))
        {
            can_see(mob, rndm) ? strcpy_s(t, MAX_INPUT_LENGTH, him_her[rndm->sex])
                               : strcpy_s(t, MAX_INPUT_LENGTH, "someone's");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "its'");
    ...
    case 'L':
        if (rndm && !char_died(rndm))
        {
            can_see(mob, rndm) ? strcpy_s(t, MAX_INPUT_LENGTH, his_her[rndm->sex])
                               : strcpy_s(t, MAX_INPUT_LENGTH, "someone");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "its");
    ...
    case 'o':
        if (obj && !obj_extracted(obj))
        {
            if (can_see_obj(mob, obj))
                ...
                strcpy_s(t, MAX_INPUT_LENGTH, "something");
        }
        else
            strcpy_s(t, MAX_INPUT_LENGTH, "something");
        break;
    ...
    case 'O':
        ...
        {
            if (can_see_obj(mob, obj))
                strcpy_s(t, MAX_INPUT_LENGTH, obj->short_descr);
            else
                strcpy_s(t, MAX_INPUT_LENGTH, "something");
        }
        else
    ...
    case 'P':
        ...
        {
            if (can_see_obj(mob, v_obj))
                strcpy_s(t, MAX_INPUT_LENGTH, v_obj->short_descr);
            else
                strcpy_s(t, MAX_INPUT_LENGTH, "something");
        }
        else
    ...
    case 'q':
        if (actor && !char_died(actor))
        {
            ...
            if (!IS_NPC(actor))
            {
                strcpy_s(t, MAX_INPUT_LENGTH, PERS(actor, mob));
                *t = UPPER(*t);
            }
        }
        else
            ...
            /*
                      switch ( *( obj->name ) )
                  {
                    case 'a': case 'e': case 'i':
                        case 'o': case 'u': strcpy_s( t, "an" );
                      break;
                        default: strcpy_s( t, "a" );
                      }
            */
        }
        else
...
export void mpsleep_update()
{
    ...
    while (mpsleep) /* Find progs to continue */
    {
        if (--mpsleep->timer <= 0)
        {
            ...
            mprog_driver(mpsleep->com_list, mpsleep->mob, mpsleep->actor, mpsleep->obj, mpsleep->vo,
                         mpsleep->single_step);
 ...
 */
export void mprog_wordlist_check(char* arg, CHAR_DATA* mob, CHAR_DATA* actor, OBJ_DATA* obj, void* vo, int type)
{
 ...
 * trigger calls.
 */
export void mprog_act_trigger(char* buf, CHAR_DATA* mob, CHAR_DATA* ch, OBJ_DATA* obj, void* vo)
{
...
export void mprog_bribe_trigger(CHAR_DATA* mob, CHAR_DATA* ch, int amount)
{
    ...
    if (IS_NPC(mob) && (mob->pIndexData->progtypes & BRIBE_PROG))
    {
        ...
        sprintf_s(buf, obj->short_descr, amount);
...
export void mprog_death_trigger(CHAR_DATA* killer, CHAR_DATA* mob)
{
...
export void mprog_entry_trigger(CHAR_DATA* mob)
{
...
export void mprog_fight_trigger(CHAR_DATA* mob, CHAR_DATA* ch)
{
...
export void mprog_give_trigger(CHAR_DATA* mob, CHAR_DATA* ch, OBJ_DATA* obj)
{
...
export void mprog_greet_trigger(CHAR_DATA* ch)
{
    CHAR_DATA *vmob, *vmob_next;

#ifdef DEBUG
    char buf[MAX_STRING_LENGTH];
    sprintf_s(buf, "mprog_greet_trigger -> %s", ch->name);
    log_string(buf);
#endif
...
export void mprog_hitprcnt_trigger(CHAR_DATA* mob, CHAR_DATA* ch)
{
...
export void mprog_random_trigger(CHAR_DATA* mob)
{
...
export void mprog_time_trigger(CHAR_DATA* mob)
{
...
export void mprog_hour_trigger(CHAR_DATA* mob)
{
...
export void mprog_speech_trigger(char* txt, CHAR_DATA* actor)
{
...
export void mprog_script_trigger(CHAR_DATA* mob)
{
    MPROG_DATA* mprg;
...
export void oprog_script_trigger(OBJ_DATA* obj)
{
    MPROG_DATA* mprg;
...
void set_supermob(OBJ_DATA* obj)
{
    ...
    sprintf_s(buf, "Object #%d", obj->pIndexData->vnum);
...
export int oprog_custom_trigger(const char* command, const char* argument, CHAR_DATA* ch)
{
    ...
    {
        if (vobj != nullptr && vobj->pIndexData != nullptr)
        {
    ...
    if (!found)
        return 0;
    set_supermob(vobj);
    mprog_driver(mprg->comlist, supermob, ch, vobj, nullptr, false);
    set_supermob(vobj);
    return 1;
}

export int mprog_custom_trigger(const char* command, const char* argument, CHAR_DATA* ch)
{
    ROOM_INDEX_DATA* room = nullptr;
    MPROG_DATA* mprg = nullptr;
    ;
    ...
    {
        if (vmob != nullptr && vmob->pIndexData != nullptr)
    ...
    if (!found)
        return 0;
    mprog_driver(mprg->comlist, vmob, ch, nullptr, nullptr, false);
    return 1;
}

export int rprog_custom_trigger(const char* command, const char* argument, CHAR_DATA* ch)
{
    ROOM_INDEX_DATA* room;
    MPROG_DATA* mprg;
    char pcom[MAX_STRING_LENGTH];
    ...
    if (!found)
        return 0;
    rset_supermob(room);
    mprog_driver(mprg->comlist, supermob, ch, nullptr, nullptr, false);
    release_supermob();
    return 1;
}
...
export void oprog_greet_trigger(CHAR_DATA* ch)
{
    OBJ_DATA* vobj;
...
export void oprog_speech_trigger(char* txt, CHAR_DATA* ch)
{
    OBJ_DATA* vobj;
 ...
 * after it
 */
export void oprog_random_trigger(OBJ_DATA* obj)
{
 ...
 */
export void oprog_wear_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
...
export bool oprog_use_trigger(CHAR_DATA* ch, OBJ_DATA* obj, CHAR_DATA* vict, OBJ_DATA* targ, void* vo)
{
    bool executed = false;
 ...
 * if !ch
 */
export void oprog_remove_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
 ...
 */
export void oprog_sac_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
 ...
 */
export void oprog_get_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
 ...
 */
export void oprog_damage_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
 ...
 */
export void oprog_repair_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
 ...
 * do a if(!ch) return right after
 */
export void oprog_drop_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
 ...
 */
export void oprog_examine_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
 ...
 *  see note there
 */
export void oprog_pull_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
 ...
 *  see note there
 */
export void oprog_push_trigger(CHAR_DATA* ch, OBJ_DATA* obj)
{
...
export void obj_act_add(OBJ_DATA* obj);
export void oprog_act_trigger(char* buf, OBJ_DATA* mobj, CHAR_DATA* ch, OBJ_DATA* obj, void* vo)
{
...
export void rset_supermob(ROOM_INDEX_DATA* room)
{
    char buf[200];
    ...
    if (room)
    {
        ...
        sprintf_s(buf, "Room #%d", room->vnum);
        STRFREE(supermob->description);
        supermob->description = STRALLOC(buf);
...
export void rprog_act_trigger(char* buf, ROOM_INDEX_DATA* room, CHAR_DATA* ch, OBJ_DATA* obj, void* vo)
{
...
export void rprog_leave_trigger(CHAR_DATA* ch)
{
...
export void rprog_enter_trigger(CHAR_DATA* ch)
{
...
export void rprog_sleep_trigger(CHAR_DATA* ch)
{
...
export void rprog_rest_trigger(CHAR_DATA* ch)
{
...
export void rprog_rfight_trigger(CHAR_DATA* ch)
{
...
export void rprog_death_trigger(CHAR_DATA* killer, CHAR_DATA* ch)
{
...
export void rprog_speech_trigger(char* txt, CHAR_DATA* ch)
{
...
export void rprog_random_trigger(CHAR_DATA* ch)
{
...
export void rprog_time_trigger(CHAR_DATA* ch)
{
...
export void rprog_hour_trigger(CHAR_DATA* ch)
{
...
export void progbug(const char* str, CHAR_DATA* mob)
{
    char buf[MAX_STRING_LENGTH];
    ...
    if (mob->pIndexData->vnum == 3)
    {
        ...
        sprintf_s(buf, "%s, %s.", str, mob->description == nullptr ? "(unknown)" : mob->description);
    }
    else
    {
        sprintf_s(buf, "%s, Mob #%d.", str, mob->pIndexData->vnum);
    }
...
export void room_act_update(void)
{
    ...
    {
        ROOM_INDEX_DATA* room = reinterpret_cast<ROOM_INDEX_DATA*>(runner->vo);
...
export void obj_act_update(void)
{
    ACT_PROG_DATA* runner;
    MPROG_ACT_LIST* mpact;
    ...
    {
        OBJ_DATA* obj = reinterpret_cast<OBJ_DATA*>(runner->vo);
