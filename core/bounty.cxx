...
void do_rembounty(CHAR_DATA* ch, char* argument)
{
    ...
    sprintf_s(buf, "The bounty on %s has been removed!", argument);
...
void disintegration(CHAR_DATA* ch, CHAR_DATA* victim, long amount)
{
    ...
    sprintf_s(buf, "Someone has added %ld credits to the bounty on %s.", amount, victim->name);
...
void claim_disintegration(CHAR_DATA* ch, CHAR_DATA* victim)
{
    ...
    if (victim->pcdata && victim->pcdata->clan)
    {
        ...
        if (victim->pcdata->clan && (!str_cmp(victim->name, clan->leader) || !str_cmp(victim->name, clan->number1) ||
    ...
    if (bounty == nullptr)
    {
        ...
        sprintf_s(buf, "%s is Dead!", victim->name);
        echo_to_all(AT_RED, buf, 0);
        return;
    }
    ...
    exp = URANGE(1, bounty->amount + xp_compute(ch, victim),
                 (exp_level(ch->skill_level[HUNTING_ABILITY] + 1) - exp_level(ch->skill_level[HUNTING_ABILITY])));
    gain_exp(ch, exp, HUNTING_ABILITY);
    ...
    sprintf_s(buf, "The disintegration bounty on %s has been claimed!", victim->name);
    echo_to_all(AT_RED, buf, 0);
    sprintf_s(buf, "%s is Dead!", victim->name);
    echo_to_all(AT_RED, buf, 0);
