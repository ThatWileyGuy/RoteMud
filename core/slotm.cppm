...
void do_playslots(CHAR_DATA* ch, char* argument)
{
    ...
    if (amount < minsbet)
    {
        sprintf_s(errbuf, "&WYou must insert at least &Y%d &Wcredit(s)!\n\r\n\r", minsbet);
    ...
    if (amount > maxsbet)
    {
        sprintf_s(errbuf, "&WWhoah....the max bet is &Y%d &Wcredit(s)!\n\r\n\r", maxsbet);
        send_to_char(errbuf, ch);
        playslots_help(ch);
        return;
    }
...
void printslots(CHAR_DATA* ch, sh_int* slot1, sh_int* slot2, sh_int* slot3)
{
    ...
    strcpy_s(slots1, get_slot_string(&(*slot1)));

    // slot 2 checks for type
    strcpy_s(slots2, get_slot_string(&(*slot2)));

    // slot 3 checks for type
    strcpy_s(slots3, get_slot_string(&(*slot3)));

    send_to_char("\n\r&c*********************************************************&w&W\n\r", ch);
    sprintf_s(buf, "   &c***&w&W  %10.10s   &c***&w&W  %10.10s   &c***&w&W  %10.10s   &c***&w&W       \n\r", slots1,
              slots2, slots3);
    send_to_char(buf, ch);
    send_to_char("&c*********************************************************&w&W\n\r", ch);
    return;
...
void printwinnings(CHAR_DATA* ch, sh_int* amount, sh_int* winningsmult)
{
    ...
    if ((*winningsmult) > 0)
    {
        sprintf_s(buf, "&BWINNER, &WYou won &Y%d &Wcredit(s)&w&W\n\r", ((*winningsmult) * (*amount)));
        send_to_char(buf, ch);
        ch->gold += ((*winningsmult) * (*amount));
    }
    else
    {
        sprintf_s(buf, "&RNo luck. &WYou lose &Y%d &Wcredit(s)\n\r", (*amount));
        send_to_char(buf, ch);
    }
