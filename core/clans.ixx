...
export CLAN_DATA* first_clan;
CLAN_DATA* last_clan;

export SENATE_DATA* first_senator;
SENATE_DATA* last_senator;

export PLANET_DATA* first_planet;
export PLANET_DATA* last_planet;
 ...
 */
export CLAN_DATA* get_clan(const char* name)
{
    CLAN_DATA* clan;
...
void write_clan_list()
{
    ...
    sprintf_s(filename, "%s%s", CLAN_DIR, CLAN_LIST);
 ...
 */
export void save_clan(CLAN_DATA* clan)
{
    FILE* fp;
    char filename[256];
...
void fread_clan(CLAN_DATA* clan, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        case 'A':
            ...
            KEY("Adjective", clan->adjective, fread_string(fp));
            break;
        ...
        if (!fMatch)
        {
            sprintf_s(buf, "Fread_clan: no match: %s", word);
            bug(buf, 0);
        }
    }
}
    ...
    if ((fp = fopen(filename, "r")) != NULL)
    {
        ...
        for (;;)
        {
            ...
            else
            {
                ...
                sprintf_s(buf, "Load_clan_file: bad section: %s.", word);
                bug(buf, 0);
                break;
            }
        }
...
export void load_clans()
{
    ...
    for (;;)
    {
        ...
        {
            sprintf_s(buf, "Cannot load clan file: %s", filename);
            bug(buf, 0);
        }
    }
    fclose(fpList);
...
void do_setclan(CHAR_DATA* ch, char* argument)
{
    ...
    if (!strcmp(arg2, "type"))
    {
        ...
        if (!str_cmp(argument, "corporation"))
...
void do_buytroops(CHAR_DATA* ch, char* argument)
{
    ...
    clan->troops += num;
    clan->funds -= cost;
    sprintf_s(buf, "&RYou have purchased %d ground troops for %s.\n\r", num, clan->name);
    send_to_char(buf, ch);
    save_clan(clan);
}
...
void do_gatherclans(CHAR_DATA* ch, char* argument)
{
    ...
    for (clan = first_clan; clan; clan = clan->next)
    {
        ...
        ch_printf(ch, "  &O%-20s %-10s %-10s %-10s %-2d %-2d %-3d %-3d %ld\n\r", "Main", clan->leader, clan->number1,
                  clan->number2, clan->spacecraft, clan->vehicles, clan->members, clan->troops, clan->funds);
        if (clan->first_subclan)
        {
            CLAN_DATA* subclan;

            for (subclan = clan->first_subclan; subclan; subclan = subclan->next_subclan)
                ch_printf(ch, "  &O%-20s %-10s %-10s %-10s %-2d %-2d %-3d %-3d %ld\n\r", subclan->name, subclan->leader,
                          subclan->number1, subclan->number2, subclan->spacecraft, subclan->vehicles, subclan->members,
                          subclan->troops, subclan->funds);
        }
        ch_printf(ch, "\n\r");
        count++;
    }
    ...
    for (clan = first_clan; clan; clan = clan->next)
    {
        ...
        ch_printf(ch, "&Y%-22s &O%-10s %-10s %-10s %-2d %-2d %-3d %ld\n\r", clan->name, clan->leader, clan->number1,
                  clan->number2, clan->spacecraft, clan->vehicles, clan->members, clan->funds);
        count++;
    }
...
void do_clanbuytroops(CHAR_DATA* ch, char* argument)
{
    ...
    if ((ch->pcdata && ch->pcdata->bestowments && is_name("clanbuyship", ch->pcdata->bestowments)) ||
        !str_cmp(ch->name, ch->pcdata->clan->leader))
        ;
    else
    {
...
void do_war(CHAR_DATA* ch, char* argument)
{
    ...
    if (nifty_is_name(wclan->name, clan->atwar))
    {
        ...
        sprintf_s(buf, "%s has declared a ceasefire with %s!", clan->name, wclan->name);
    ...
    strcpy_s(buf, clan->atwar);
    strcat_s(buf, "\n\r ");
    strcat_s(buf, wclan->name);
    strcat_s(buf, " ");

    STRFREE(clan->atwar);
    clan->atwar = STRALLOC(buf);

    sprintf_s(buf, "%s has declared war on %s!", clan->name, wclan->name);
...
void do_checkwar(CHAR_DATA* ch, char* argument)
{
    ...
    for (clan = first_clan; clan; clan = clan->next)
    {
        if (!str_cmp(clan->name, "Neutral"))
            continue;
        sprintf_s(buf, "%s %-3.3s |", buf, clan->acro ? clan->acro : "NNE");
        sprintf_s(buf2, "%s-----+", buf2);
    }
    ...
    for (clan = first_clan; clan; clan = clan->next)
    {
        if (!str_cmp(clan->name, "Neutral"))
            continue;
        buf[0] = '\0';
        sprintf_s(buf, "| %-3.3s |", clan->acro ? clan->acro : "NNE");
        for (clan2 = first_clan; clan2; clan2 = clan2->next)
        {
            ...
            if (nifty_is_name(clan2->name, clan->atwar))
                sprintf_s(buf, "%s &R%-3.3s &w|", buf, "Yes");
            else
                sprintf_s(buf, "%s &G%-3.3s &w|", buf, "No");
        }
        clancnt++;
        send_to_char(buf, ch);
        send_to_char("\n\r", ch);
...
void do_empower(CHAR_DATA* ch, char* argument)
{
    ...
    {
        sprintf_s(buf, "%s %s", victim->pcdata->bestowments, arg2);
        DISPOSE(victim->pcdata->bestowments);
    ...
    {
        sprintf_s(buf, "%s outlaw unoutlaw", victim->pcdata->bestowments);
        DISPOSE(victim->pcdata->bestowments);
        victim->pcdata->bestowments = str_dup(buf);
...
void save_senate()
{
    /*
        ...
        sprintf_s( filename, "%s%s", SYSTEM_DIR, BOUNTY_LIST );
        fpout = fopen( filename, "w" );
        if ( !fpout )
        {
...
void do_outlaw(CHAR_DATA* ch, char* argument)
{
    ...
    if ((ch->pcdata && ch->pcdata->bestowments && is_name("outlaw", ch->pcdata->bestowments)) ||
        !str_cmp(ch->name, clan->leader) || !str_cmp(ch->name, clan->number1) || !str_cmp(ch->name, clan->number2))
        ;
    else
    {
    ...
    ch_printf(victim,
              "&W&RA slight buzz comes over your comlink and you hear, 'Attention all citizens, %s has been banned "
              "from %s's planets until further notice. Guards will attack on sight. Thank you.'\n\r",
...
void do_unoutlaw(CHAR_DATA* ch, char* argument)
{
    ...
    if ((ch->pcdata && ch->pcdata->bestowments && is_name("unoutlaw", ch->pcdata->bestowments)) ||
        !str_cmp(ch->name, clan->leader) || !str_cmp(ch->name, clan->number1) || !str_cmp(ch->name, clan->number2))
        ;
    else
    {
    ...
    ch_printf(victim,
              "&W&GA slight buzz comes over your comlink and you hear, 'Attention all citizens, %s has been absolved "
              "of any crimes created on %s's planets. They are no longer wanted. Thank you.'\n\r",
              victim->name, clan->name);
        ...
        for (subclan = clan->first_subclan; subclan; subclan = subclan->next_subclan)
        {
            ...
            if (clan->leader[0] != 0)
                ch_printf(ch, "&x&W|       &BLeader&W: %-12.12s       &BSpacecraft&W: %-2.2s         |\n\r",
                          subclan->leader, subclan->spacecraft);
            else
            ...
            ch_printf(ch, "&x&W|        &BFirst&W: %-12.12s         &BSpacecraft&W: %-2.2s       |\n\r",
                      subclan->number1, subclan->spacecraft);
            ch_printf(ch, "&x&W|       &BSecond&W: %-12.12s          &BMembers&W: %-2.2s         |\n\r",
                      subclan->number2, subclan->members);
        }
    }
...
void do_clans(CHAR_DATA* ch, char* argument)
{
    ...
    for (clan = first_clan; clan; clan = clan->next)
    {
        ...
        /*
            ch_printf( ch, "  &WDivision             Leaders             Spacecraft/Vehicles/Members/Troops/Funds\n\r");
            ch_printf( ch, "  &O%-20s %-10s %-10s %-10s %-2d %-2d %-3d %-3d %ld\n\r",
                      "Main", clan->leader, clan->number1, clan->number2, clan->spacecraft, clan->vehicles,
           clan->members, clan->troops, clan->funds ); if ( clan->first_subclan )
            {
               CLAN_DATA *subclan;
...
void do_members(CHAR_DATA* ch, char* argument)
{
    ...
    for (;;)
    {
        ...
        if (!str_cmp(buf, clan->leader))
            sprintf_s(prefix, "%s[&W1%s]&W", color, color);
        else if (!str_cmp(buf, clan->number1))
            sprintf_s(prefix, "%s[&W2%s]&W", color, color);
        else if (!str_cmp(buf, clan->number2))
            sprintf_s(prefix, "%s[&W3%s]&W", color, color);
        else
            sprintf_s(prefix, "   &W");

        sprintf_s(display, "%s%-20s", prefix, buf);
