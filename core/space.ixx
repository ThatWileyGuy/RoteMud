...
export SPACE_DATA* first_starsystem;
...
export void update_space()
{
    ...
    for (ship = first_ship; ship; ship = ship->next)
    {
        ...
        if ((ship->clazz >= SHIP_CRUISER) && ship->target0 == NULL)
        {
 ...
 */
export SPACE_DATA* starsystem_from_name(const char* name)
{
...
export void echo_to_ship(int color, SHIP_DATA* ship, const char* argument)
{
    int room;
...
export void echo_to_cockpit(int color, SHIP_DATA* ship, const char* argument)
{
    int room;
...
export void echo_to_system(int color, SHIP_DATA* ship, const char* argument, SHIP_DATA* ignore)
{
    SHIP_DATA* target;
...
export long int get_ship_value(SHIP_DATA* ship)
{
    long int price;
...
export SHIP_DATA* ship_in_room(ROOM_INDEX_DATA* room, char* name)
{
    SHIP_DATA* ship;
 ...
 */
export SHIP_DATA* get_ship(char* name)
{
    SHIP_DATA* ship;
 ...
 */
export SHIP_DATA* get_ship_here(char* name, SPACE_DATA* starsystem)
{
    SHIP_DATA* ship;
...
export SHIP_DATA* ship_from_cockpit(int vnum)
{
    SHIP_DATA* ship;
...
export SHIP_DATA* ship_from_pilotseat(int vnum)
{
    SHIP_DATA* ship;
...
export SHIP_DATA* ship_from_gunseat(int vnum)
{
    SHIP_DATA* ship;
...
export SHIP_DATA* ship_from_engine(int vnum)
{
    SHIP_DATA* ship;
...
export void save_ship(SHIP_DATA* ship)
{
    FILE* fp;
    char filename[256];
    char buf[MAX_STRING_LENGTH];
...
bool load_ship_file(const char* shipfile)
{
    ...
    else
    {
        ...
        else if ((pRoomIndex = get_room_index(ship->lastdoc)) != NULL && ship->clazz <= SHIP_CORVETTE)
        {
...
export void resetship(SHIP_DATA* ship)
{
    untargetship(ship);
    ship->shipstate = SHIP_READY;
...
export void ship_to_starsystem(SHIP_DATA* ship, SPACE_DATA* starsystem)
{
    if (starsystem == NULL)
        return;
...
export bool check_pilot(CHAR_DATA* ch, SHIP_DATA* ship)
{
...
export bool extract_ship(SHIP_DATA* ship)
{
    ROOM_INDEX_DATA* room;
...
void launchship(SHIP_DATA* ship)
{
    ...
    if ((room = get_room_index(ship->lastdoc)) != NULL && room->area && room->area->planet &&
 ...
 */
{
    ...
    if (!str_cmp(arg1, "stop"))
    {
        ...
        if (number_percent() < chance)
        {
            ...
            act(AT_PLAIN, "$n begins the tractor beam sequence.", ch, NULL, argument, TO_ROOM);
            echo_to_ship(AT_YELLOW, target, "The ship shudders as a tractorbeam locks on.");
...
export bool autofly(SHIP_DATA* ship)
{
...
export SHIP_DATA* ship_from_room(int vnum)
{
    SHIP_DATA* ship;
    int sRoom;
...
void do_load_cargo(CHAR_DATA* ch, char* argument)
{
    ...
    if (arg1[0] == '\0')
    {
        act(AT_PLAIN, "SYNTAX - load_cargo <ship> <type>", ch, NULL, NULL, TO_CHAR);
        return;
    }
