...
void do_chedit(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg1, "delete"))
    {
        ...
        {
            sprintf_s(buf, "Valid changes are from 1 to %d.\n\r", maxChanges);
            send_to_char(buf, ch);
            return;
        }
        delete_change(num);
...
char *change_justify(char *pszText, int iAlignment)
{
    ...
    {
        strcpy_s(s_szResult, "BUG: Justified string cannot be less than 10 characters long.");
        return (&s_szResult[0]);
    }
...
void do_changes(CHAR_DATA *ch, char *argument)
{
    ...
    for (i = 0; i < maxChanges; i++)
    {
        ...
        sprintf_s(buf, "&R[&w%2d&Y]&W %-9s &C*%-6s &d%-55s&d\n\r", (i + 1), changes_table[i].coder,
                  changes_table[i].date, changes_table[i].change);
        // change_justify( changes_table[i].change, 55 ));
        send_to_char(buf, ch);
    }
    send_to_char("&b---------------------------------------------------------------------------\n\r", ch);
    sprintf_s(buf, "&YThere is a total of &R[ &w%3d &R] &Ychanges in the database.&d\n\r", maxChanges);
    send_to_char(buf, ch);
    send_to_char("&YAlso see&B: &R'&wchanges all&R'&Y for a list of all the changes.&z\n\r", ch);
    send_to_char("&b---------------------------------------------------------------------------\n\r", ch);
    sprintf_s(buf, "&YThere is a total of &R[ &w%2d &R] &Ynew changes that have been added today.&d\n\r", today);
    send_to_char(buf, ch);
    send_to_char("&b---------------------------------------------------------------------------\n\r", ch);
    return;
}
