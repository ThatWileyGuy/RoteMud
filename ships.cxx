...
void do_buymobship(CHAR_DATA* ch, char* argument)
{
	...
	if (ship_prototypes[x].clazz != 1 && ship_prototypes[x].clazz != 2 && ship_prototypes[x].clazz != 3 && ship_prototypes[x].clazz != 9)
	{
	...
	switch (ship_type)
	{
	default: sprintf_s(shipname, "Mobile Ship MS"); break;
		// NR
	case 0: sprintf_s(shipname, "X-Wing Snubfighter MXW"); break;
	case 1: sprintf_s(shipname, "A-Wing Scout MAW"); break;
	case 2: sprintf_s(shipname, "B-Wing Heavy Fighter MBW"); break;
	case 3: sprintf_s(shipname, "Y-Wing Bomber MYB"); break;
	case 4: sprintf_s(shipname, "K-Wing Heavy Bomber MKW"); break;
		// Imp
	case 6: sprintf_s(shipname, "TIE Fighter MTF"); break;
	case 7: sprintf_s(shipname, "TIE Bomber MTB"); break;
	case 8: sprintf_s(shipname, "TIE Defender MTD"); break;
	case 9: sprintf_s(shipname, "XM-1 Missileboat MXM"); break;
	case 10: sprintf_s(shipname, "XG-1 Assault Gunboat MXG"); break;
	}
	...
	for (sship = system->first_ship; sship; sship = sship->next_in_starsystem)
	{
		...
		if (sship->clazz >= SHIP_CRUISER) { fcap = TRUE; caps++; }
	}
	if (!fcap)
	{
...
void do_upgradeship(CHAR_DATA* ch, char* argument)
{
	...
	if (!str_cmp(arg2, "hyperdrive"))
	{
		...
		if (ship->hyperspeed == 0)
		{
			...
			{
				send_to_char("Shipyard workers scramble out onto the dock and work furiously to install your hyperdrive\n\r", ch);
				ship->hyperspeed = 3;
		...
		{
			send_to_char("Shipyard workers scramble out onto the dock and work furiously to upgrade your hyperdrive\n\r", ch);
			ship->hyperspeed++;
			ch->gold = ch->gold - ship->hypercost;
			save_ship(ship);
			return;
	...
	if (!str_cmp(arg2, "weapons"))
	{
		...
		if (!str_cmp(arg3, "ptype"))
		{
			...
			else
			{
				...
				{
					send_to_char("Shipyard workers scramble out onto the dock and work furiously to upgrade your 	weapons\n\r", ch);
					ship->primaryType++;
					ship->upptype++;
		...
		if (!str_cmp(arg3, "pcount"))
		{
			...
			{
				send_to_char("Shipyard workers scramble out onto the dock and work furiously to add a gun to your ship\n\r", ch);
				ship->primaryCount++;
				ship->uppcountmax++;
		...
		if (!str_cmp(arg3, "stype"))
		{
			...
			else
			{
				...
				{
					send_to_char("Shipyard workers scramble out onto the dock and work furiously to upgrade your 	secondary weapons\n\r", ch);
					ship->secondaryType++;
					ship->upstype++;
		...
		if (!str_cmp(arg3, "scount"))
		{
			...
			{
				send_to_char("Shipyard workers scramble out onto the dock and work furiously to add a secondary gun to your ship\n\r", ch);
				ship->secondaryCount++;
				ship->upscountmax++;
	...
	if (!str_cmp(arg2, "tractor"))
	{
		...
		else
		{
			...
			{
				send_to_char("Shipyard workers scramble out onto the dock and work furiously to install your tractor beam", ch);
				ship->tractorbeam = 1;
	...
	if (!str_cmp(arg2, "energy"))
	{
		...
		{
			send_to_char("Shipyard workers scramble out onto the dock and work furiously to upgrade your fuel capacity", ch);
...
void do_shipups(CHAR_DATA* ch, char* argument)
{
	...
	ch_printf(ch, "&GShields:&CLevel: &w%d &CMaximum Level: &w%d &CCost: &w%d", ship->upshield, ship->upshieldmax, ship->upshieldcost);
}
...
const char* primary_beam_name_proto(int shiptype)
{
	...
	{
		sprintf_s(pbname, "%d %s", ship_prototypes[shiptype].primaryCount,
...
const char* secondary_beam_name_proto(int shiptype)
{
	...
	{
		sprintf_s(pbname, "%d %s", ship_prototypes[shiptype].secondaryCount,
...
void do_shipstat(CHAR_DATA* ch, char* argument)
{
	...
	bool playercanviewship = !(shiphasclan && (!playerhasclan || !(!str_cmp(ch->pcdata->clan->name, ship_prototypes[shiptype].clan) || (ch->pcdata->clan->mainclan != NULL && !str_cmp(ch->pcdata->clan->mainclan->name, ship_prototypes[shiptype].clan)))));
	...
	if (ship_prototypes[shiptype].primaryType > 0) sprintf_s(buf1, "%d", ship_prototypes[shiptype].primaryType);
	else sprintf_s(buf1, "&RNone.");
	if (ship_prototypes[shiptype].secondaryType > 0) sprintf_s(buf2, "%d", ship_prototypes[shiptype].secondaryType);
	else sprintf_s(buf2, "&RNone.");
	if (ship_prototypes[shiptype].range_weapons[0] > 0) sprintf_s(buf3, "%d", ship_prototypes[shiptype].range_weapons[0]);
	else sprintf_s(buf3, "&RNone.");
	if (ship_prototypes[shiptype].shields > 0) sprintf_s(buf4, "%d", ship_prototypes[shiptype].shields);
	else sprintf_s(buf4, "&RNone.");
	if (ship_prototypes[shiptype].range_weapons[1] > 0) sprintf_s(buf5, "%d", ship_prototypes[shiptype].range_weapons[1]);
	else sprintf_s(buf5, "&RNone.");
	if (ship_prototypes[shiptype].hyperspeed > 0) sprintf_s(buf6, "Class %d", ship_prototypes[shiptype].hyperspeed);
	else sprintf_s(buf6, "&RNone.");
	if (ship_prototypes[shiptype].range_weapons[2] > 0) sprintf_s(buf7, "%d", ship_prototypes[shiptype].range_weapons[2]);
	else sprintf_s(buf7, "&RNone.");
	if (ship_prototypes[shiptype].maxbombs > 0) sprintf_s(buf8, "%d", ship_prototypes[shiptype].maxbombs);
	else sprintf_s(buf8, "&RNone.");
	if (ship_prototypes[shiptype].turrets > 0) sprintf_s(buf9, "%d", ship_prototypes[shiptype].turrets);
	else sprintf_s(buf9, "&RNone.");
	if (ship_prototypes[shiptype].chaff > 0) sprintf_s(buf10, "%d", ship_prototypes[shiptype].chaff);
	else sprintf_s(buf10, "&RNone.");
...
void do_shiplist(CHAR_DATA* ch, char* argument)
{
	...
	for (x = 0; x < NUM_PROTOTYPES; x++) {
		...
		bool playerhasclan = (ch->pcdata->clan != NULL && str_cmp(ch->pcdata->clan->name, ""));
		bool playercanviewship = !(shiphasclan && (!playerhasclan || !(!str_cmp(ch->pcdata->clan->name, ship_prototypes[x].clan) || (ch->pcdata->clan->mainclan != NULL && !str_cmp(ch->pcdata->clan->mainclan->name, ship_prototypes[x].clan)))));
		char type[255] = "Classified";
		if (playercanviewship) {
			sprintf_s(type, "%s %s",
				shiphasclan ? ch->pcdata->clan->adjective : "Civilian",
				...
				ship_prototypes[x].clazz == LAND_VEHICLE ? "Land vehicle" : "Unknown");
