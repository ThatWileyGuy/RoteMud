...
void de_equip_char(CHAR_DATA* ch)
{
	...
	for (obj = ch->first_carrying; obj; obj = obj->next_content)
		if (obj->wear_loc > -1 && obj->wear_loc < MAX_WEAR)
		{
			...
			if (x == MAX_LAYERS)
			{
				sprintf_s(buf, "%s had on more than %d layers of clothing in one location (%d): %s",
...
void save_clone(CHAR_DATA* ch)
{
	...
	sprintf_s(strsave, "%s%c/%s.clone", PLAYER_DIR, tolower(ch->name[0]),
		capitalize(ch->name));
...
bool load_char_obj(DESCRIPTOR_DATA* d, char* name, bool preload)
{
	...
	ch->build = 0;
	found = FALSE;
	sprintf_s(strsave, "%s%c/%s", PLAYER_DIR, tolower(name[0]),
		capitalize(name));
	if (stat(strsave, &fst) != -1)
	{
		if (fst.st_size == 0)
		{
			sprintf_s(strsave, "%s%c/%s", BACKUP_DIR, tolower(name[0]),
				capitalize(name));
			send_to_char("Restoring your backup player file...", ch);
		}
		else
		{
			sprintf_s(buf, "%s player data for: %s (%dK)",
	...
	for (; ; )
	{
		...
		case 'C':
			...
			if (!str_cmp(word, "Clan"))
			{
				...
				{
					sprintf_s(buf, "Warning: the organization %s no longer exists, and therefore you no longer\n\rbelong to that organization.\n\r",
						ch->pcdata->clan_name);
					send_to_char(buf, ch);
...
void save_profile(CHAR_DATA* ch)
{
	...
	else
	{
		...
		if (ch->pcdata->image && ch->pcdata->image[0] != '\0')
			...
			sprintf_s(image, "../grx/noimage.gif");

		if (ch->sex == 1)
			sprintf_s(sex, "Male");
		else if (ch->sex == 2)
			sprintf_s(sex, "Female");
		else
			sprintf_s(sex, "Neutral");

		if (ch->pcdata->email && ch->pcdata->email[0] != '\0')
			sprintf_s(email, "%s", ch->pcdata->email);
		else
		{
		...
		if (ch->pcdata->homepage && ch->pcdata->homepage[0] != '\0')
		{
			sprintf_s(homepage, "%s", show_tilde(ch->pcdata->homepage));
		}
		else
		{
		...
		if (ch->pcdata->screenname && ch->pcdata->screenname[0] != '\0')
			sprintf_s(aimname, "%s", ch->pcdata->screenname);
		else
		{
		...
		if (ch->description && ch->description[0] != '\0')
			sprintf_s(desc, "%s", ch->description);
		else
			sprintf_s(desc, "Not Specified.");

		if (ch->pcdata->bio && ch->pcdata->bio[0] != '\0')
			sprintf_s(bio, "%s", ch->pcdata->bio);
		else
			sprintf_s(bio, "None.");
