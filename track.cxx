...
void found_prey( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	switch( number_bits( 2 ) )
 	{
	case 0: sprintf_s( buf, "Don't make me find you, %s!", victname );
		do_say( ch, buf );
	        break;
	case 1: act( AT_ACTION, "$n sniffs around the room for $N.", ch, NULL, victim, TO_NOTVICT );
		...
		do_say( ch, buf );
		break;
	case 2: sprintf_s( buf, "I'm going to tear %s apart!", victname );
		do_yell( ch, buf );
		break;
	...
	case 0:	do_say( ch, "C'mon out, you coward!" );
		...
		do_yell( ch, buf );
		break;
	case 1: sprintf_s( buf, "Let's take this outside, %s", victname );
		do_say( ch, buf );
		break;
	case 2: sprintf_s( buf, "%s is a yellow-bellied wimp!", victname );
		do_yell( ch, buf );
		break;
	...
	return;
     ...
     switch( number_bits( 2 ) )
     {
     case 0: sprintf_s( buf, "Your blood is mine, %s!", victname );
	     do_yell( ch, buf);
	     break;
     case 1: sprintf_s( buf, "Alas, we meet again, %s!", victname );
     	     do_say( ch, buf );
     	     break;
     case 2: sprintf_s( buf, "What do you want on your tombstone, %s?", victname );
     	     do_say( ch, buf );
     	     break;
