...
void char_to_room(CHAR_DATA* ch, ROOM_INDEX_DATA* pRoomIndex)
{
	...
	if (!pRoomIndex)
	{
		...
		sprintf_s(buf, "Char_to_room: %s -> NULL room!  Putting char in limbo (%d)",
...
const char* affect_bit_name(int vector)
{
	...
	buf[0] = '\0';
	if (vector & AFF_BLIND) strcat_s(buf, " blind");
	if (vector & AFF_INVISIBLE) strcat_s(buf, " invisible");
	if (vector & AFF_DETECT_EVIL) strcat_s(buf, " detect_evil");
	if (vector & AFF_DETECT_INVIS) strcat_s(buf, " detect_invis");
	if (vector & AFF_DETECT_MAGIC) strcat_s(buf, " detect_magic");
	if (vector & AFF_DETECT_HIDDEN) strcat_s(buf, " detect_hidden");
	if (vector & AFF_WEAKEN) strcat_s(buf, " weaken");
	if (vector & AFF_SANCTUARY) strcat_s(buf, " sanctuary");
	if (vector & AFF_FAERIE_FIRE) strcat_s(buf, " faerie_fire");
	if (vector & AFF_INFRARED) strcat_s(buf, " infrared");
	if (vector & AFF_CURSE) strcat_s(buf, " curse");
	if (vector & AFF_COVER_TRAIL) strcat_s(buf, " cover trail");
	if (vector & AFF_POISON) strcat_s(buf, " poison");
	if (vector & AFF_PROTECT) strcat_s(buf, " protect");
	if (vector & AFF_PARALYSIS) strcat_s(buf, " paralysis");
	if (vector & AFF_SLEEP) strcat_s(buf, " sleep");
	if (vector & AFF_SNEAK) strcat_s(buf, " sneak");
	if (vector & AFF_HIDE) strcat_s(buf, " hide");
	if (vector & AFF_CHARM) strcat_s(buf, " charm");
	if (vector & AFF_POSSESS) strcat_s(buf, " possess");
	if (vector & AFF_FLYING) strcat_s(buf, " flying");
	if (vector & AFF_PASS_DOOR) strcat_s(buf, " pass_door");
	if (vector & AFF_FLOATING) strcat_s(buf, " floating");
	if (vector & AFF_TRUESIGHT) strcat_s(buf, " true_sight");
	if (vector & AFF_DETECTTRAPS) strcat_s(buf, " detect_traps");
	if (vector & AFF_SCRYING) strcat_s(buf, " scrying");
	if (vector & AFF_FIRESHIELD) strcat_s(buf, " fireshield");
	if (vector & AFF_SHOCKSHIELD) strcat_s(buf, " shockshield");
	if (vector & AFF_ICESHIELD) strcat_s(buf, " iceshield");
	if (vector & AFF_POSSESS) strcat_s(buf, " possess");
	if (vector & AFF_BERSERK) strcat_s(buf, " berserk");
	if (vector & AFF_AQUA_BREATH) strcat_s(buf, " aqua_breath");
...
const char* extra_bit_name(int extra_flags)
{
	...
	buf[0] = '\0';
	if (extra_flags & ITEM_GLOW) strcat_s(buf, " glow");
	if (extra_flags & ITEM_HUM) strcat_s(buf, " hum");
	if (extra_flags & ITEM_DARK) strcat_s(buf, " dark");
	if (extra_flags & ITEM_HUTT_SIZE) strcat_s(buf, " hutt_size");
	if (extra_flags & ITEM_CONTRABAND) strcat_s(buf, " contraband");
	if (extra_flags & ITEM_INVIS) strcat_s(buf, " invis");
	if (extra_flags & ITEM_MAGIC) strcat_s(buf, " magic");
	if (extra_flags & ITEM_NODROP) strcat_s(buf, " nodrop");
	if (extra_flags & ITEM_BLESS) strcat_s(buf, " bless");
	if (extra_flags & ITEM_ANTI_GOOD) strcat_s(buf, " anti-good");
	if (extra_flags & ITEM_ANTI_EVIL) strcat_s(buf, " anti-evil");
	if (extra_flags & ITEM_ANTI_NEUTRAL) strcat_s(buf, " anti-neutral");
	if (extra_flags & ITEM_NOREMOVE) strcat_s(buf, " noremove");
	if (extra_flags & ITEM_INVENTORY) strcat_s(buf, " inventory");
	if (extra_flags & ITEM_DEATHROT) strcat_s(buf, " deathrot");
	if (extra_flags & ITEM_ANTI_SOLDIER) strcat_s(buf, " anti-soldier");
	if (extra_flags & ITEM_ANTI_THIEF) strcat_s(buf, " anti-thief");
	if (extra_flags & ITEM_ANTI_HUNTER) strcat_s(buf, " anti-hunter");
	if (extra_flags & ITEM_ANTI_JEDI) strcat_s(buf, " anti-jedi");
	if (extra_flags & ITEM_ANTI_SITH) strcat_s(buf, " anti-sith");
	if (extra_flags & ITEM_ANTI_PILOT) strcat_s(buf, " anti-pilot");
	if (extra_flags & ITEM_SMALL_SIZE) strcat_s(buf, " small_size");
	if (extra_flags & ITEM_LARGE_SIZE) strcat_s(buf, " large_size");
	if (extra_flags & ITEM_DONATION) strcat_s(buf, " donation");
	if (extra_flags & ITEM_CLANOBJECT) strcat_s(buf, " clan");
	if (extra_flags & ITEM_ANTI_CITIZEN) strcat_s(buf, " anti-citizen");
	if (extra_flags & ITEM_PROTOTYPE) strcat_s(buf, " prototype");
	if (extra_flags & ITEM_HUMAN_SIZE) strcat_s(buf, " human_size");
	return (buf[0] != '\0') ? buf + 1 : "none";
}
...
const char* magic_bit_name(int magic_flags)
{
	...
	buf[0] = '\0';
	if (magic_flags & ITEM_RETURNING) strcat_s(buf, " returning");
	return (buf[0] != '\0') ? buf + 1 : "none";
}
