...
void mobile_update(void)
{
	...
	for (ch = last_char; ch; ch = gch_prev)
	{
		...
		{
			sprintf_s(buf, "FATAL: Mobile_update: %s->prev->next doesn't point to ch.",
				ch->name);
			bug(buf, 0);
			...
			|| obj->item_type == ITEM_DROID_CORPSE)
		{
			...
			if (obj->timer > 0 && obj->value[2] > timerfrac)
			{
				...
				if (obj->item_type == ITEM_DROID_CORPSE)
					sprintf_s(buf, d_corpse_descs[UMIN(timerfrac - 1, 4)],
						bufptr);
				else
					sprintf_s(buf, corpse_descs[UMIN(timerfrac - 1, 4)],
						capitalize(bufptr));
...
void auth_update(void)
{
	...
	for (d = first_descriptor; d; d = d->next)
	{
		...
		if (victim && IS_WAITING_FOR_AUTH(victim))
		{
			...
			sprintf_s(log_buf, " %s@%s new %s", victim->name,
...
void reboot_check(time_t reset)
{
	...
	if (new_boot_time_t <= current_time)
	{
		...
		echo_to_all(AT_AUCTION, "You are forced from these realms by a strong "
...
void auction_update(void)
{
	...
	case 3: /* SOLD! */
		...
		{
			sprintf_s(buf, "&W%s&W is &wSOLD&W for &Y%d&W credits.",
				auction->item->short_descr,
				auction->bet);
			talk_auction(buf);

			act(AT_ACTION, "&wA courier arrives, and hands you $p.",
		...
		else /* not sold */
		{
			...
			talk_auction(buf);
			act(AT_ACTION, "&wA courier arrives, and returns $p to you.",
