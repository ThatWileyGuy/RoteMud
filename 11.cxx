...
char const* primary_beam_name(SHIP_DATA* ship)
{
	...
	{
		sprintf_s(bname, "%d %s", ship->primaryCount,
...
char const* secondary_beam_name(SHIP_DATA* ship)
{
	...
	{
		sprintf_s(bname, "%d %s", ship->secondaryCount,
			...
			if (pexit->vdir == dir)
			{
				if (IS_SET(pexit->exit_info, EX_CLOSED)) // Door in the way?
				{
					if (!IS_SET(pexit->exit_info, EX_BASHPROOF))
					{
						...
						{
							if (rch == ch)  continue;
							sprintf_s(buf, "%s launches a missile %s, and it EXPLODES upon impact of the doorway!", PERS(ch, rch), dtxt);
					...
					else // is bashproof
					{
						...
						{
							if (rch == ch)  continue;
							sprintf_s(buf, "&w%s fires a missile %s, and it explodes upon impact of the doorway!", PERS(ch, rch), dtxt);
							send_to_char(buf, rch);
						}
						send_to_char("&RThe missile flies directly into the doorway, and EXPLODES!\n\r", ch);
