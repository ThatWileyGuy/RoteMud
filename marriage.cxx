...
void do_marry( CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_NPC(vic1))
   {
     sprintf_s(buf, "%s appears to be a mob. Why marry it off to anyone?\n\r", vic1->name);
     send_to_char(buf, ch);
     return;
   }
  ...
  if(vic1->top_level < 5)
   {
     sprintf_s(buf, "%s is to low a level to get married.\n\r", vic1->name);
     send_to_char(buf, ch);
     return;
   }
  ...
  */ 
  sprintf_s(buf, "&R(&WMarriage&R} &W%s: I am very pleased to announce that %s and %s are now married.", ch->name, vic1->name, vic2->name);
...
void do_divorce( CHAR_DATA *ch, char *argument )
{
  ...
  if (vic1->pcdata->spouse != vic2->pcdata->spouse)
   {
     sprintf_s(buf, "%s is not married to %s\n\r", vic1->name, vic2->name);
     send_to_char(buf, ch);
     return;
   }
  ...
  vic1->gold -= 500;
  vic2->gold -= 500;
    sprintf_s(buf, "&R(&WMarriage&R} &W%s: It is my sad duty to anounce that %s and %s are now divorced.", ch->name, vic1->name, vic2->name);
}   
...
void do_propose( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ch == victim ){
    ...
    if (ch->sex == SEX_MALE)
    sprintf_s(buf, "What a moron. %s proposed to himself.", ch->name);
    else if (ch->sex == SEX_FEMALE)
    sprintf_s(buf, "What a moron. %s proposed to herself.", ch->name);
    else
    sprintf_s(buf, "What a moron. %s proposed to itself.", ch->name);
...
if ( ch->position == POS_FIGHTING)
  ...
  if ((victim = get_char_world (ch, ch->pcdata->spouse)) == NULL)
    ...
    sprintf_s(buf, "&G[&PSpouse&G] &Pto %s: &W%s\n\r", victim->name, argument);
    send_to_char(buf, ch); 
    sprintf_s(buf, "&G[&PSpouse&G] &P%s Spouse talks: &W%s\n\r", ch->name, argument); 
