	...
	case ITEM_WIELD:
		...
		if (tmpobj)
		{
			if (can_dual(ch))
			{
				...
				{
					send_to_char("You need two hands to wield a dual lightsaber and your already wielding a weapon.\n\r", ch);
					return;
				}
...
void do_auction(CHAR_DATA* ch, char* argument)
{
	...
	*/
	if (arg1[0] == '\0')
	{
		if (auction->item != NULL)
		{
			...
			if (auction->bet > 0)
				...
				sprintf_s(buf, "zero");
			ch_printf(ch, "\n\r&W++\n\r&z||&w Item:&G %s  &wType:&G %s\n\r", obj->short_descr, aoran(item_type_name(obj)));
	...
	if (IS_IMMORTAL(ch) && !str_cmp(arg1, "stop")) {
		...
		{
			set_char_color(AT_AUCTION, ch);
			sprintf_s(buf, "Sale of %s has been stopped by an Immortal.",
...
void obj_fall(OBJ_DATA* obj, bool through)
{
		...
		&& !IS_OBJ_STAT(obj, ITEM_MAGIC))
	{
		...
		{
			sprintf_s(buf, "Object falling into same room, room %d",
				to_room->vnum);
			bug(buf, 0);
			extract_obj(obj);
			return;
