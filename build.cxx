...
"import", "storagedock", "r14", "r15", "r16", "r17", "r18", "r19", "r20", "r21", "r22", "r23",
	...
	*/
	...
	if (!str_cmp(arg2, "hitnumdie"))
	{
		...
		if (IS_NPC(victim) && IS_SET(victim->act, ACT_PROTOTYPE))
		{
			...
			victim->max_hit = generate_hp(victim->top_level, victim->pIndexData->hitnodice, victim->pIndexData->hitsizedice, victim->pIndexData->hitplus);
		}
...
void do_redit(CHAR_DATA* ch, char* argument)
{
	...
	if (!str_cmp(arg, "bexit"))
	{
		...
		if (vnum)
		{
			sprintf_s(tmpcmd, "%d redit exit %d %s %s",
				vnum,
				rev_dir[edir],
				rvnum,
				argument);
