...
void do_restrict(CHAR_DATA* ch, char* argument)
{
	...
	if (found)
	{
		...
		ch_printf(ch, "You restrict %s to level %d\n\r",
			cmd->name, level);
		sprintf_s(buf, "%s restricting %s to level %d",
...
void do_authorize(CHAR_DATA* ch, char* argument)
{
	...
	if (arg2[0] == '\0' || !str_cmp(arg2, "accept") || !str_cmp(arg2, "yes"))
	{
		...
		sprintf_s(buf, "%s authorized %s", ch->name,
			victim->name);
	...
	else if (!str_cmp(arg2, "no") || !str_cmp(arg2, "deny"))
	{
		...
		sprintf_s(buf, "%s denied authorization to %s", ch->name,
			victim->name);
		to_channel(buf, CHANNEL_MONITOR, "Monitor", ch->top_level);
	...
	{
		sprintf_s(buf, "%s has denied %s's name", ch->name,
			victim->name);
		to_channel(buf, CHANNEL_MONITOR, "Monitor", ch->top_level);
		ch_printf(victim,
...
void do_mstat(CHAR_DATA* ch, char* argument)
{
	...
	if (!IS_NPC(victim) && victim->desc)
	{
		...
		ch_printf(ch, "&W&z| &GDescriptor&W: %-3d                                       &GAuthedBy&W: %-12s &z|\n\r", -1, victim->pcdata->authed_by[0] != '\0' ? victim->pcdata->authed_by : "(unknown)");
		ch_printf(ch, "&W&z+------------------------------------------------------------------------------+\n\r");
	}
...
void do_oldmstat(CHAR_DATA* ch, char* argument)
{
	...
	if (get_trust(ch) >= LEVEL_GOD && !IS_NPC(victim) && victim->desc)
		ch_printf(ch, "User: %s@%s   Descriptor: %d   Trust: %d   AuthedBy: %s\n\r",
			victim->desc->user, victim->desc->host, -1, // TODO no more descriptors!
...
void do_owhere(CHAR_DATA* ch, char* argument)
{
	...
	if (arg1[0] != '\0' && !str_prefix(arg1, "nesthunt"))
	{
		...
		sprintf_s(buf, "[%5d] %-28s in ", obj->pIndexData->vnum,
			obj_short(obj));
		if (obj->carried_by)
	...
	for (obj = first_object; obj; obj = obj->next)
	{
		...
		sprintf_s(buf, "(%3d) [%5d] %-28s in ", ++icnt, obj->pIndexData->vnum,
			obj_short(obj));
		if (obj->carried_by)
...
void do_loadup(CHAR_DATA* ch, char* argument)
{
	...
	sprintf_s(fname, "%s%c/%s", PLAYER_DIR, tolower(name[0]),
		capitalize(name));
...
void do_destroy(CHAR_DATA* ch, char* argument)
{
	...
	sprintf_s(buf, "%s%c/%s", PLAYER_DIR, tolower(argument[0]),
		capitalize(argument));
	sprintf_s(buf2, "%s%c/%s", BACKUP_DIR, tolower(argument[0]),
		capitalize(argument));
	if (!rename(buf, buf2))
	{
...
const char* name_expand(CHAR_DATA* ch)
{
	...
	{
		strcpy_s(outbuf, ""); /* Do not return NULL, just an empty buffer */
		return outbuf;
	}
...
void do_restorefile(CHAR_DATA* ch, char* argument)
{
	...
	sprintf_s(fname, "%s%c/%s", BACKUP_DIR, tolower(arg[0]),
		capitalize(arg));
	if (stat(fname, &fst) != -1)
	{
...
void ostat_plus(CHAR_DATA* ch, OBJ_DATA* obj)
{
	...
	******/
	...
	case ITEM_ARMOR:
		 ...
		 *****/
		if (dam >= 10) strcpy_s(buf, "Superb condition");
		else if (dam == 9) strcpy_s(buf, "Very good condition");
		else if (dam == 8) strcpy_s(buf, "Good shape");
		else if (dam == 7) strcpy_s(buf, "Showing a bit of wear");
		else if (dam == 6) strcpy_s(buf, "A little run down");
		else if (dam == 5) strcpy_s(buf, "In need of repair");
		else if (dam == 4) strcpy_s(buf, "In great need of repair");
		else if (dam == 3) strcpy_s(buf, "In dire need of repair");
		else if (dam == 2) strcpy_s(buf, "Very badly worn");
		else if (dam == 1) strcpy_s(buf, "Practically worthless");
		else if (dam <= 0) strcpy_s(buf, "Broken");
		strcat_s(buf, "\n\r");
		send_to_char(buf, ch);
		break;
		/*
	...
	case ITEM_FOOD:
		...
		if (obj->timer > 0 && obj->value[1] > 0)
			...
			dam = 10;
		if (dam >= 10) strcpy_s(buf, "It is fresh.");
		else if (dam == 9) strcpy_s(buf, "It is nearly fresh.");
		else if (dam == 8) strcpy_s(buf, "It is perfectly fine.");
		else if (dam == 7) strcpy_s(buf, "It looks good.");
		else if (dam == 6) strcpy_s(buf, "It looks ok.");
		else if (dam == 5) strcpy_s(buf, "It is a little stale.");
		else if (dam == 4) strcpy_s(buf, "It is a bit stale.");
		else if (dam == 3) strcpy_s(buf, "It smells slightly off.");
		else if (dam == 2) strcpy_s(buf, "It smells quite rank.");
		else if (dam == 1) strcpy_s(buf, "It smells revolting!");
		else if (dam <= 0) strcpy_s(buf, "It is crawling with maggots!");
		strcat_s(buf, "\n\r");
		send_to_char(buf, ch);
		if (obj->value[4])
