	...
	for (victim = ch->in_room->first_person; victim; victim = v_next)
	{
		...
		for (obj = victim->last_carrying; obj; obj = obj->prev_content)
		{
			if (obj->pIndexData->item_type == ITEM_WEAPON)
			{
				if (victim != ch && can_see(ch, victim) && can_see_obj(ch, obj))
				{
					sprintf_s(buf, "Weapons are banned from non-military usage. I'm going to have to confiscate %s.", obj->short_descr);
					do_say(ch, buf);
					if (obj->wear_loc != WEAR_NONE)
						remove_obj(victim, obj->wear_loc, TRUE);
					separate_obj(obj);
...
bool spec_guardian(CHAR_DATA* ch)
{
	...
	{
		sprintf_s(buf, "%s is a %s!  As well as a COWARD!",
			victim->name, crime);
		do_yell(ch, buf);
		return TRUE;
	}
	...
	if (victim)
	{
		sprintf_s(buf, "%s is a %s!  PROTECT THE INNOCENT!!",
			victim->name, crime);
		do_shout(ch, buf);
		multi_hit(ch, victim, TYPE_UNDEFINED);
		return TRUE;
