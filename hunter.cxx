...
void do_plantbug(CHAR_DATA* ch, char* argument)
{
    ...
    chance = number_percent() - UMIN(0, (get_curr_lck(ch) - 14)) + UMIN(0, (get_curr_lck(victim) - 13));
...
void do_unbind(CHAR_DATA* ch, char* argument)
{
    ...
    else
    {
        ...
        sprintf_s(buf, "%s unbinding %s: has no bothwrists object!", ch->name, victim->name);
        bug(buf);
        return;
    }
