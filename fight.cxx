...
void violence_update( void )
{
	...
	{
	    sprintf_s( buf, "FATAL: violence_update: %s->prev->next doesn't point to ch.",
		ch->name );
	    bug( buf, 0 );	    
	...
	if ( !ch->in_room || !ch->name )
	{
	    ...
	    sprintf_s( buf, "ch: %d  ch->in_room: %d  ch->prev: %d  ch->next: %d",
	    	(int) ch, (int) ch->in_room, (int) ch->prev, (int) ch->next );
	    log_string( buf );
	    log_string( lastplayercmd );
	    if ( lst_ch )
	      sprintf_s( buf, "lst_ch: %d  lst_ch->prev: %d  lst_ch->next: %d",
	...
	{
	   sprintf_s( buf, "violence_update: %s fighting %s in a SAFE room.",
	   	ch->name, victim->name );
	   log_string( buf );
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if ( !npcvict )
	{
	    sprintf_s( log_buf, "%s killed by %s at %d",
		victim->name,
...
//    int x;
    ...
    */
	...
	if ( !ch->master )
	{
	    ...
	    sprintf_s( buf, "Check_killer: %s bad AFF_CHARM",
		IS_NPC(ch) ? ch->short_descr : ch->name );
	    bug( buf, 0 );
...
void set_fighting( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	sprintf_s( buf, "Set_fighting: %s -> %s (already fighting %s)",
...
/* swreality chnages begin here */
  ...
  sprintf_s( buf, "%s%c/%s", PLAYER_DIR, tolower(arg[0]),
          capitalize( arg ) );
  sprintf_s( buf2, "%s%c/%s", BACKUP_DIR, tolower(arg[0]),
          capitalize( arg ) );
  
  rename( buf, buf2 );

  sprintf_s( buf, "%s%c/%s.clone", PLAYER_DIR, tolower(arg[0]),
          capitalize( arg ) );
  sprintf_s( buf2, "%s%c/%s", PLAYER_DIR, tolower(arg[0]),
          capitalize( arg ) );
