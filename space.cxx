...
const int   station_vnum[MAX_STATION] =
{
	215 , 216 , 217 , 218 , 219 , 220 , 221 ,222 , 223 , 224
};

const char* station_name[MAX_STATION] =
{
...
void update_bus()
{
	...
	case 0:
		...
		{
			sprintf_s(buf, "An electronic voice says, 'Cannot land at %s ... it seems to have dissapeared.'", bus_stop[bus_planet]);
		...
		if (target != NULL && !target->starsystem)
		{
			sprintf_s(buf, "An electronic voice says, 'Cannot land at %s ... it seems to have disappeared.'", bus_stop[bus_planet]);
	...
	for (ship = first_ship; ship; ship = ship->next)
	{
		...
		if (ship->starsystem)
		{
			...
			for (target = ship->starsystem->first_ship; target; target = target->next_in_starsystem)
			{
				...
				{
					sprintf_s(buf, "Proximity alert: %s  %.0f %.0f %.0f"
			...
			{
				sprintf_s(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->star1,
			...
			{
				sprintf_s(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->star2,
			...
			{
				sprintf_s(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->planet1,
			...
			{
				sprintf_s(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->planet2,
			...
			{
				sprintf_s(buf, "Proximity alert: %s  %d %d %d", ship->starsystem->planet3,
		...
		if (ship->target0)
		{
			sprintf_s(buf, "%s   %.0f %.0f %.0f", ship->target0->name,
		...
		if (ship->target1)
		{
			sprintf_s(buf, "%s   %.0f %.0f %.0f", ship->target1->name,
		...
		if (ship->target2)
		{
			sprintf_s(buf, "%s   %.0f %.0f %.0f", ship->target2->name,
...
void do_showship(CHAR_DATA* ch, char* argument)
{
	...
	ch_printf(ch, "%s %s : %s\n\rFilename: %s\n\r",
		...
		ship->clazz == LAND_VEHICLE ? "Land vehicle" : "Unknown",
		ship->name,
		ship->filename);
...
void do_ships(CHAR_DATA* ch, char* argument)
{
	...
	if (!IS_NPC(ch))
	{
		...
		for (ship = first_ship; ship; ship = ship->next)
		{
			...
			{
				if (!ch->pcdata || !ch->pcdata->clan || str_cmp(ship->owner, ch->pcdata->clan->name) || ship->clazz > SHIP_SPACE_STATION)
					continue;
			}
...
void do_land(CHAR_DATA* ch, char* argument)
{
	...
	if (!pfound)
	{
		...
		if (target != NULL)
		{
			...
			if ((target->hanger1 &&
				(ship->clazz >= SHIP_FIGHTER && ship->clazz <= SHIP_SHUTTLE))
...
void do_buyship(CHAR_DATA* ch, char* argument)
{
	...
	if (ship->clanowner && (!ch->pcdata->clan || (str_cmp(ch->pcdata->clan->name, ship->clanowner) && str_cmp(ch->pcdata->clan->mainclan->name, ship->clanowner)))) {
...
void do_info(CHAR_DATA* ch, char* argument)
{
	...
	ch_printf(ch, "&G| &W%s %s:&w %s &G|\n\r\n\r",
		...
		target->clazz == LAND_VEHICLE ? "Land vehicle" : "Unknown",
		target->name);
	...
	if (!str_cmp(target->owner, ch->name) || !str_cmp(target->pilot, ch->name) || !str_cmp(target->copilot, ch->name))
		ch_printf(ch, "&G| &WOwner: &w%s&G || &WPilot: &w%s&G || &WCoPilot: &w%s&G |\n\r",
			target->owner[0] != '\0' ? target->owner : "None.", target->pilot[0] != '\0' ? target->pilot : "None.", target->copilot[0] != '\0' ? target->copilot : "None.");
...
void do_status(CHAR_DATA* ch, char* argument)
{
	...
	ch_printf(ch, "&zSpeed:&w %d&W/%d   &zHyperdrive: &wClass %d   %s %s\n\r",
		...
		ship->clazz >= SHIP_CRUISER ? "&zBay doors:" : "",
...
void do_bomb(CHAR_DATA* ch, char* argument)
{
	...
	sprintf_s(buf, "&GYou unleash your payload of %d bombs onto the lands of %s, and watch as the bombs make small explosions viewable from orbit.\n\r", bombs, planet->name);
	ship->bombs -= bombs;
...
void do_freeship(CHAR_DATA* ch, char* argument)
{
	...
	for (ship = first_ship; ship; ship = ship->next)
	{
		if (!str_cmp(ship->owner, "Public"))
		{
			if (ship->in_room)
			{
				sprintf_s(buf, "&z| %s%-25.25s &z| &Y%-10ld&z | &R%-34.33s &z|\n\r", ship->type == CLAN_SHIP ? "&g" : "&B", ship->name, get_ship_value(ship) / 100, ship->in_room->name);
				send_to_char(buf, ch);
			}
			else
			{
				sprintf_s(buf, "&z| %s%-25.25s &z| &Y%-10ld&z | &RSpace                              &z|\n\r", ship->type == CLAN_SHIP ? "&g" : "&B", ship->name, get_ship_value(ship) / 100);
				send_to_char(buf, ch);
			}
		}
	}
...
const char* ship_bit_name(int vector)
{
	...
	buf[0] = '\0';
	if (vector & SHIP_NOHIJACK) strcat_s(buf, " nohijack");
	if (vector & SHIP_SHIELD_BOOST) strcat_s(buf, " shield_boost");
	if (vector & SHIP_TORP_BOOST) strcat_s(buf, " torp_boost");
	if (vector & SHIP_CHAFF_BOOST) strcat_s(buf, " chaff_boost");
	if (vector & SHIP_HULL_BOOST) strcat_s(buf, " hull_boost");
	if (vector & SHIP_LASER_BOOST) strcat_s(buf, " laser_boost");
	if (vector & SHIP_MISSILE_BOOST) strcat_s(buf, " missile_boost");
	if (vector & SHIP_ROCKET_BOOST) strcat_s(buf, " rocket_boost");
	if (vector & SHIP_SIMULATOR) strcat_s(buf, " simulator");
	if (vector & SHIP_NODESTROY) strcat_s(buf, " nodestroy");
	if (vector & SHIP_NOSLICER) strcat_s(buf, " noslicer");
	if (vector & XSHIP_ION_LASERS) strcat_s(buf, " ion_lasers");
	if (vector & XSHIP_ION_DRIVE) strcat_s(buf, " ion_drive");
	if (vector & XSHIP_ION_ION) strcat_s(buf, " ion_ion");
	if (vector & XSHIP_ION_TURRET1) strcat_s(buf, " ion_turret1");
	if (vector & XSHIP_ION_TURRET2) strcat_s(buf, " ion_turret2");
	if (vector & XSHIP_ION_TURRET3) strcat_s(buf, " ion_turret3");
	if (vector & XSHIP_ION_TURRET4) strcat_s(buf, " ion_turret4");
	if (vector & XSHIP_ION_TURRET5) strcat_s(buf, " ion_turret5");
	if (vector & XSHIP_ION_TURRET6) strcat_s(buf, " ion_turret6");
	if (vector & XSHIP_ION_TURRET7) strcat_s(buf, " ion_turret7");
	if (vector & XSHIP_ION_TURRET8) strcat_s(buf, " ion_turret8");
	if (vector & XSHIP_ION_TURRET9) strcat_s(buf, " ion_turret9");
	if (vector & XSHIP_ION_TURRET10) strcat_s(buf, " ion_turret10");
	if (vector & SHIP_RESPAWN) strcat_s(buf, " respawn");
	if (vector & XSHIP_ION_MISSILES) strcat_s(buf, " ion_missiles");
	if (vector & XSHIP_ION_HYPER) strcat_s(buf, " ion_hyper");
	if (vector & SHIP_CLOAK) strcat_s(buf, " cloak");
 ...
 * Allows player to use comlink to get status on any of his ships with a slave circuit.  */
{
	...
	ch_printf(ch, "&zSpeed:&w %d&W/%d   &zHyperdrive: &wClass %d %s %s\n\r", ship->currspeed, ship->realspeed, ship->hyperspeed, ship->clazz >= SHIP_CRUISER ? "&zBay doors:" : "", (ship->clazz >= SHIP_CRUISER && ship->bayopen == TRUE) ? "&wOpen" : (ship->clazz >= SHIP_CRUISER && ship->bayopen == FALSE) ? "&wClosed" : "");
...
void do_split_s(CHAR_DATA* ch, char* argument)
{
	...
	if (!ship->target0)
	{
		...
		else
		{
			...
			ship->hz = number_range(-2000, 3000);
			act(AT_PLAIN, "$n performs a split-S.", ch, NULL, NULL, TO_ROOM);
			sprintf_s(buf, "%s quickly rolls down and around.",
				ship->name);
			echo_to_system(AT_YELLOW, ship, buf, NULL);
			learn_from_success(ch, gsn_split_s);
			WAIT_STATE(ch, 8);
	...
	if (!is_facing(ship, ship->target0)) {
		ch_printf(ch, "&GYou perform a split-S, and face %s.\n\r",
			ship->target0->name);
		sprintf_s(buf, "%s quickly rolls down and around, facing %s.",
		...
		ch_printf(ch, "&GYou perform a split-S, turning away from %s.\n\r",
			ship->target0->name);
		sprintf_s(buf, "%s quickly rolls down and around, turning away from %s.",
			ship->name, ship->target0->name);
		ship->hx = 0 - ship->target0->vx - ship->vx;
		ship->hy = 0 - ship->target0->vy - ship->vy;
		ship->hz = 0 - ship->target0->vz - ship->vz;
...
void do_load_cargo(CHAR_DATA* ch, char* argument)
{
	...
	if (arg1[0] == '\0')
	{
		act(AT_PLAIN, "SYNTAX - load_cargo <ship> <type>", ch, NULL, NULL,
			TO_CHAR);
		return;
	}
